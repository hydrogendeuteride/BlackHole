...
namespace GameRuntime
{
    ...
    void Runtime::run(IGameCallbacks *game)
    {
        ...
        while (!_quit_requested)
        {
            ...
            bool ui_capture_keyboard = _renderer->ui() && _renderer->ui()->wantCaptureKeyboard();
            ...
            if (game && input)
            {
                consumed_mouse_button_mask = game->on_pre_input();
            }

            const uint32_t right_mouse_mask = mouse_button_mask(MouseButton::Right);
            if (_renderer->picking() && input)
            {
                _renderer->picking()->process_input(*input,
                                                    ui_capture_mouse,
                                                    consumed_mouse_button_mask);
            }

            // --- Camera input (if not captured by UI) --- //
            if (_renderer->_sceneManager && input)
            {
                const bool game_capture_mouse =
                        (consumed_mouse_button_mask & ~right_mouse_mask) != 0u;
                if (_renderer->_sceneManager->getCameraRig().mode() == CameraMode::Free &&
                    input->state().mouse_pressed(MouseButton::Right) &&
                    !ui_capture_mouse && consumed_mouse_button_mask == 0 &&
                    _renderer->ui() && _renderer->ui()->releaseKeyboardFocus())
                {
                    // The scene owns this gesture; finish UI editing before camera movement.
                    ui_capture_keyboard = false;
                }
                _renderer->_sceneManager->getCameraRig().process_input(*input,
                                                                        ui_capture_keyboard,
                                                                        ui_capture_mouse || game_capture_mouse);
            }
            ...
            _renderer->prepare_scene_for_frame();
