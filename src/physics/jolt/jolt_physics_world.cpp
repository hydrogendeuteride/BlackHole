...
namespace Physics
{
    ...
    namespace
    {
        ...
        bool try_make_jolt_body_id(const BodyId id, JPH::BodyID &out)
        {
            if (!id.is_valid() || (id.value & JPH::BodyID::cBroadPhaseBit) != 0)
            {
                out = JPH::BodyID();
                return false;
            }

            out = JPH::BodyID(id.value);
            return true;
        }

        bool is_jolt_body_added(const JPH::BodyInterface &bi, const BodyId id, JPH::BodyID &out)
        {
            return try_make_jolt_body_id(id, out) && bi.IsAdded(out);
        }
        ...
        JPH::MassProperties to_jolt_mass_properties(
                const MassProperties &properties)
        {
            JPH::MassProperties out{};
            out.mMass = static_cast<float>(properties.mass_kg);
            out.mInertia = JPH::Mat44::sZero();
            for (uint32_t row = 0; row < 3; ++row)
            {
                for (uint32_t column = 0; column < 3; ++column)
                {
                    out.mInertia(row, column) = static_cast<float>(
                            properties.inertia_local_kgm2[column][row]);
                }
            }
        ...
        glm::dmat3 from_jolt_inertia(const JPH::Mat44 &inertia)
        {
            glm::dmat3 out(0.0);
            for (uint32_t row = 0; row < 3; ++row)
            {
                ...
                {
                    out[column][row] =
                            static_cast<double>(inertia(row, column));
                }
            }
    ...
    BodyId JoltPhysicsWorld::create_body(const BodySettings &settings)
    {
        ...
        if (jolt_shape && settings.has_explicit_mass_properties &&
            settings.mass_properties.valid())
        {
            const glm::dvec3 center =
                    settings.mass_properties.center_of_mass_local_m;
            const JPH::Vec3 desired_center(
                    static_cast<float>(center.x),
                    static_cast<float>(center.y),
                    static_cast<float>(center.z));
            const JPH::Vec3 offset =
                    desired_center - jolt_shape->GetCenterOfMass();
            if (offset.LengthSq() > 1.0e-12f)
            {
                jolt_shape = new JPH::OffsetCenterOfMassShape(
                        jolt_shape.GetPtr(), offset);
            }
        }
        std::scoped_lock layer_lock(_layer_mutex);
        ...
        if (motion == MotionType::Dynamic)
        {
            ...
            if (settings.has_explicit_mass_properties &&
                settings.mass_properties.valid())
            {
                body_settings.mOverrideMassProperties =
                        JPH::EOverrideMassProperties::MassAndInertiaProvided;
                body_settings.mMassPropertiesOverride =
                        to_jolt_mass_properties(settings.mass_properties);
            }
            else if (custom_mass_properties.has_value())
            {
    ...
    {
        JPH::BodyID jolt_id;
        if (!_initialized || !try_make_jolt_body_id(id, jolt_id))
        {
            return;
        }

        auto erase_body_metadata = [this, id]() {
            {
                std::scoped_lock lock(_callbacks_mutex);
    ...
    glm::vec3 JoltPhysicsWorld::get_linear_velocity(BodyId id) const
    {
        ...
        std::scoped_lock lock(_layer_mutex);
        if (!is_jolt_body_added(_physics_system.GetBodyInterface(), id, jolt_id))
        {
            return glm::vec3(0.0f);
        }
    ...
    glm::vec3 JoltPhysicsWorld::get_unit_inertia(BodyId id) const
    {
        MassProperties properties{};
        if (!get_mass_properties(id, properties))
        {
            return glm::vec3(0.0f);
        }
        return glm::vec3(
                properties.inertia_local_kgm2[0][0] / properties.mass_kg,
                properties.inertia_local_kgm2[1][1] / properties.mass_kg,
                properties.inertia_local_kgm2[2][2] / properties.mass_kg);
    }

    bool JoltPhysicsWorld::get_mass_properties(
            const BodyId id,
            MassProperties &out) const
    {
        out = {};
        JPH::BodyID jolt_id;
        if (!_initialized || !try_make_jolt_body_id(id, jolt_id))
        ...
        std::scoped_lock lock(_layer_mutex);
        JPH::BodyLockRead body_lock(_physics_system.GetBodyLockInterface(), jolt_id);
        ...
        const float inv_mass = motion->GetInverseMass();
        if (!(inv_mass > 0.0f) || !std::isfinite(inv_mass))
        {
            return false;
        }

        JPH::Mat44 inertia = JPH::Mat44::sZero();
        if (!inertia.SetInversed3x3(motion->GetLocalSpaceInverseInertia()))
        {
            return false;
        }

        const JPH::Vec3 center = body.GetShape()->GetCenterOfMass();
        out = {
                .mass_kg = 1.0 / static_cast<double>(inv_mass),
                .center_of_mass_local_m = {
                        static_cast<double>(center.GetX()),
                        static_cast<double>(center.GetY()),
                        static_cast<double>(center.GetZ()),
                },
                .inertia_local_kgm2 = from_jolt_inertia(inertia),
        };
        return out.valid();
    }
    ...
    bool JoltPhysicsWorld::set_mass(BodyId id, float mass)
    {
        JPH::BodyID jolt_id;
        if (!_initialized || !(mass > 0.0f) || !std::isfinite(mass) ||
            !try_make_jolt_body_id(id, jolt_id))
        {
            return false;
        }

        std::scoped_lock lock(_layer_mutex);
        JPH::BodyLockWrite body_lock(_physics_system.GetBodyLockInterface(), jolt_id);
        if (!body_lock.Succeeded())
        {
            return false;
        }

        JPH::Body &body = body_lock.GetBody();
        if (!body.IsDynamic() || body.GetShape() == nullptr)
        {
            return false;
        }
    ...
    bool JoltPhysicsWorld::set_mass_properties(
            const BodyId id,
            const MassProperties &properties)
    {
        JPH::BodyID jolt_id;
        if (!_initialized || !properties.valid() ||
            !try_make_jolt_body_id(id, jolt_id))
        {
            return false;
        }

        CollisionShape source_shape{};
        {
            std::scoped_lock lock(_debug_bodies_mutex);
            const auto it = _debug_bodies.find(id.value);
            if (it == _debug_bodies.end())
            {
                return false;
            }
            source_shape = it->second.shape;
        }

        JPH::RefConst<JPH::Shape> shape = create_jolt_shape(source_shape);
        if (!shape)
        {
            return false;
        }
        const glm::dvec3 center = properties.center_of_mass_local_m;
        const JPH::Vec3 desired_center(
                static_cast<float>(center.x),
                static_cast<float>(center.y),
                static_cast<float>(center.z));
        const JPH::Vec3 offset = desired_center - shape->GetCenterOfMass();
        if (offset.LengthSq() > 1.0e-12f)
        {
        ...
        if (!body_interface.IsAdded(jolt_id))
        {
            return false;
        }
        ...
        JPH::BodyLockWrite body_lock(
                _physics_system.GetBodyLockInterface(), jolt_id);
        if (!body_lock.Succeeded())
        {
            return false;
        }
        JPH::Body &body = body_lock.GetBody();
        JPH::MotionProperties *motion = body.GetMotionProperties();
        if (!body.IsDynamic() || motion == nullptr)
        {
            return false;
        }
        motion->SetMassProperties(
                motion->GetAllowedDOFs(),
                to_jolt_mass_properties(properties));
        return true;
    }
    ...
    bool JoltPhysicsWorld::set_motion_type(BodyId id, MotionType motion_type)
    {
        ...
        std::scoped_lock layer_lock(_layer_mutex);
        JPH::BodyInterface &bi = _physics_system.GetBodyInterface();
        if (!is_jolt_body_added(bi, id, jolt_id))
        {
            return false;
        }
                                   ...
                                   const SweepOptions &options) const
    {
        ...
        const JPH::RShapeCast shape_cast =
                JPH::RShapeCast::sFromWorldTransform(jolt_shape.GetPtr(),
                                                     JPH::Vec3::sOne(),
                                                     world_transform,
                                                     cast_dir);
        const JPH::RVec3 base_offset = world_transform.GetTranslation();
    ...
    {
        JPH::BodyID jolt_id;
        if (!try_make_jolt_body_id(id, jolt_id))
        {
            return;
        }
    ...
    JointId JoltPhysicsWorld::create_fixed_joint(BodyId body_a, BodyId body_b)
    {
        JPH::BodyID jolt_a;
        JPH::BodyID jolt_b;
        if (!_initialized || !try_make_jolt_body_id(body_a, jolt_a) || !try_make_jolt_body_id(body_b, jolt_b))
        {
            return JointId{};
        }

        std::scoped_lock layer_lock(_layer_mutex);
        const JPH::BodyID bodies[2] = {jolt_a, jolt_b};
