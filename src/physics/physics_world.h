...
namespace Physics
{
    ...
    public:
        ...
        // Diagonal of the body's local inertia tensor for a unit total mass
        // (geometry only; scale by the actual mass to get kg·m²). Returns zero
        // for backends or bodies that do not provide mass properties.
        virtual glm::vec3 get_unit_inertia(BodyId id) const
        {
            (void) id;
            return glm::vec3(0.0f);
        }

        virtual bool get_mass_properties(BodyId id,
        ...
        // Runtime mass override; recomputes the inertia tensor from the body
        // shape scaled to the new mass. Returns false for unsupported backends.
        virtual bool set_mass(BodyId id, float mass)
        {
            (void) id;
            (void) mass;
            return false;
        ...
        virtual bool set_mass_properties(BodyId id,
