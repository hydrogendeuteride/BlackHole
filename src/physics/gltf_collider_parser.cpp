...
namespace Physics
{
    namespace
    {
        ...
        std::optional<Bounds> merged_mesh_bounds(const MeshNode *mesh_node)
        {
            if (!mesh_node || !mesh_node->mesh || mesh_node->mesh->surfaces.empty())
            {
                return {};
            }
            ...
            bool found = false;
            for (const GeoSurface &surface : mesh_node->mesh->surfaces)
            {
                const glm::vec3 bmin = surface.bounds.origin - surface.bounds.extents;
                const glm::vec3 bmax = surface.bounds.origin + surface.bounds.extents;
                minp = glm::min(minp, bmin);
                maxp = glm::max(maxp, bmax);
                found = true;
            }
            ...
            Bounds bounds{};
            bounds.origin = (minp + maxp) * 0.5f;
            bounds.extents = (maxp - minp) * 0.5f;
            bounds.sphereRadius = glm::length(bounds.extents);
            return bounds;
        }

        float tapered_radius_at_y(const MeshNode *mesh_node,
                                  float y,
                                  const glm::vec3 &center,
                                  const glm::vec3 &scale,
                                  float fallback)
        {
            if (!mesh_node || !mesh_node->mesh || !mesh_node->mesh->bvh)
            {
                return fallback;
            }

            constexpr float kYRelEps = 1.0e-3f;
            const float y_eps = std::max(1.0e-4f, std::abs(y) * kYRelEps);
            float radius = 0.0f;
            bool found = false;

            auto add_vertex = [&](const glm::vec3 &v) {
                if (std::abs(v.y - y) > y_eps)
                {
                    return;
                }
                const float dx = (v.x - center.x) * scale.x;
                const float dz = (v.z - center.z) * scale.z;
                radius = std::max(radius, std::sqrt(dx * dx + dz * dz));
                found = true;
            };

            for (const bvh2::PrimitiveF &prim : mesh_node->mesh->bvh->primitives)
            {
                add_vertex(glm::vec3(prim.v0.x, prim.v0.y, prim.v0.z));
                add_vertex(glm::vec3(prim.v1.x, prim.v1.y, prim.v1.z));
                add_vertex(glm::vec3(prim.v2.x, prim.v2.y, prim.v2.z));
            }

            return found ? radius : fallback;
        }
        ...
        std::optional<PrimitiveShapeVariant> make_collider_shape_from_mesh_bounds(
            GltfColliderMarkerType type,
            const MeshNode *mesh_node,
            const glm::vec3 &node_scale,
            glm::vec3 &out_center_local,
            std::string_view scene_name,
            std::string_view node_name)
        {
            const std::optional<Bounds> bounds = merged_mesh_bounds(mesh_node);
            if (!bounds.has_value())
            {
                return {};
            }

            const glm::vec3 signed_scale = node_scale;
            const glm::vec3 abs_scale = glm::abs(node_scale);
            const glm::vec3 extents = bounds->extents * abs_scale;
            const glm::vec3 size = extents * 2.0f;
            const glm::vec3 center_local = bounds->origin * signed_scale;

            auto with_center = [&](PrimitiveShapeVariant shape) -> std::optional<PrimitiveShapeVariant> {
                out_center_local = center_local;
                return shape;
            };

            if (!(size.x > 0.0f) || !(size.y > 0.0f) || !(size.z > 0.0f))
            {
                Logger::warn("[GLTF][Colliders] '{}' node '{}' mesh bounds have zero size; falling back to node scale",
                             scene_name, node_name);
                return {};
            }
            ...
            {
                case GltfColliderMarkerType::Box:
                    return with_center(BoxShape{extents});
                case GltfColliderMarkerType::Sphere:
                {
                    if (!approx_equal_rel(size.x, size.y, kUniformEps) || !approx_equal_rel(size.x, size.z, kUniformEps))
                    {
                        Logger::warn("[GLTF][Colliders] '{}' node '{}' sphere mesh bounds are non-uniform (x={}, y={}, z={}); using max radius",
                                     scene_name, node_name, size.x, size.y, size.z);
                    }
                    return with_center(SphereShape{0.5f * std::max({size.x, size.y, size.z})});
                }
                case GltfColliderMarkerType::Capsule:
                {
                    if (!approx_equal_rel(size.x, size.z, kUniformEps))
                    {
                        Logger::warn("[GLTF][Colliders] '{}' node '{}' capsule mesh bounds X/Z are non-uniform (x={}, z={}); using max radius",
                                     scene_name, node_name, size.x, size.z);
                    }
                    const float radius = 0.5f * std::max(size.x, size.z);
                    return with_center(CapsuleShape{radius, std::max(0.0f, 0.5f * size.y - radius)});
                }
                case GltfColliderMarkerType::Cylinder:
                {
                    if (!approx_equal_rel(size.x, size.z, kUniformEps))
                    {
                        Logger::warn("[GLTF][Colliders] '{}' node '{}' cylinder mesh bounds X/Z are non-uniform (x={}, z={}); using max radius",
                                     scene_name, node_name, size.x, size.z);
                    }
                    return with_center(CylinderShape{0.5f * std::max(size.x, size.z), 0.5f * size.y});
                }
                case GltfColliderMarkerType::TaperedCylinder:
                {
                    const float fallback = 0.5f * std::max(size.x, size.z);
                    const float top_y = bounds->origin.y + bounds->extents.y;
                    const float bottom_y = bounds->origin.y - bounds->extents.y;
                    const float top_radius = tapered_radius_at_y(mesh_node, top_y, bounds->origin, signed_scale, fallback);
                    const float bottom_radius = tapered_radius_at_y(mesh_node, bottom_y, bounds->origin, signed_scale, fallback);
                    if (top_radius <= 0.0f && bottom_radius <= 0.0f)
                    {
                        Logger::warn("[GLTF][Colliders] '{}' node '{}' tapered cylinder mesh bounds have zero radii; falling back to node scale",
                                     scene_name, node_name);
                        return {};
                    }
                    return with_center(TaperedCylinderShape{0.5f * size.y, top_radius, bottom_radius});
                }
                default:
                    break;
            }
            ...
            const OwnerPredicate& resolve_owner)
        {
            ...
            for (const auto& [node_name, node_ptr] : scene.nodes)
            {
                ...
                std::optional<PrimitiveShapeVariant> shape =
                    make_collider_shape_from_mesh_bounds(type, mesh_node, s, center_local, scene_name, node_name);
                if (!shape.has_value())
                {
                    shape = make_collider_shape_from_scale(type, s, scene_name, node_name);
                }
                if (!shape.has_value())
                {
                    continue;
        ...
        bool clear_existing)
    {
        ...
        std::unordered_set<std::string_view> dst_node_names;
        dst_node_names.reserve(dst_scene.nodes.size());
        std::unordered_map<std::string_view, const Node *> dst_nodes_by_name;
        dst_nodes_by_name.reserve(dst_scene.nodes.size());
        for (const auto &[name, node_ptr] : dst_scene.nodes)
        {
            if (!node_ptr)
            {
                continue;
            }
            dst_node_names.insert(name);
            dst_nodes_by_name.emplace(name, node_ptr.get());
        }
        ...
        std::unordered_map<const Node*, std::string_view> name_by_ptr;
        name_by_ptr.reserve(sidecar_scene.nodes.size());
        for (const auto& [name, node_ptr] : sidecar_scene.nodes)
        {
            if (!node_ptr) continue;
            name_by_ptr[node_ptr.get()] = name;
        }
        ...
        const std::string_view scene_name =
            sidecar_scene.debugName.empty() ? "<unnamed>" : std::string_view(sidecar_scene.debugName);

        for (const auto& [node_name, node_ptr] : sidecar_scene.nodes)
        {
            if (!node_ptr) continue;
            ...
            const std::string_view owner_name = owner_name_it->second;
            auto dst_owner_it = dst_nodes_by_name.find(owner_name);
            if (dst_owner_it == dst_nodes_by_name.end() || !dst_owner_it->second)
            {
                Logger::warn("[GLTF][Colliders] '{}' collider node '{}' owner '{}' missing in destination scene; skipping",
                             scene_name, node_name, owner_name);
                continue;
            }

            const glm::mat4 rel = glm::inverse(dst_owner_it->second->worldTransform) * node_ptr->worldTransform;
            glm::vec3 t{0.0f};
            glm::quat r{1.0f, 0.0f, 0.0f, 0.0f};
            glm::vec3 s{1.0f};
            decompose_trs_matrix(rel, t, r, s);
            ...
            t += r * center_local;
            const float child_mass = parse_collider_mass_override(node_name).value_or(0.0f);
            out_compounds[std::string(owner_name)].add_child(*shape, t, r, 0, child_mass, canonical_collider_name(node_name));
        }
    }
        ...
        bool clear_existing)
    {
        ...
        std::unordered_map<const MeshAsset *, std::shared_ptr<const TriangleMeshData>> mesh_cache;
        mesh_cache.reserve(sidecar_scene.meshes.size());

        const std::string_view scene_name = sidecar_scene.debugName.empty() ? "<unnamed>" : std::string_view(sidecar_scene.debugName);
        ...
        for (const auto &[node_name, node_ptr] : sidecar_scene.nodes)
        {
            ...
            if (!accept_node(node_name, *mesh_node))
            {
                continue;
            }
            ...
            if (dst_owner_it == dst_nodes_by_name.end() || !dst_owner_it->second)
            {
                Logger::warn("[GLTF][Colliders] '{}' mesh collider node '{}' owner '{}' missing in destination scene; skipping",
                             scene_name, node_name, owner_name);
                continue;
            }
            ...
            inst.mesh = std::move(mesh_data);
            inst.relative_transform = rel;
            out_instances[std::string(owner_name)].push_back(std::move(inst));
        }
    }
