...
void Engine::set_plain_background(bool enabled, const glm::vec3& color)
{
    if (_engine->_context)
    {
        _engine->_context->plainBackground = enabled;
        _engine->_context->backgroundColor = glm::max(color, glm::vec3(0.0f));
    }
}

bool Engine::get_plain_background(glm::vec3& color) const
{
    color = _engine->_context ? _engine->_context->backgroundColor : glm::vec3(0.0f);
    return _engine->_context && _engine->_context->plainBackground;
}
