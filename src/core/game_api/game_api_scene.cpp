...
bool Engine::set_gltf_instance_visual_rotation(const std::string& name, const glm::quat& rotation)
{
    return _engine->_sceneManager
           ? _engine->_sceneManager->setGLTFInstanceVisualRotation(name, rotation)
           : false;
}
...
{
    return _engine->_sceneManager
           ? _engine->_sceneManager->setDynamicInstanceVisualTransform(name, transform.to_matrix())
           : false;
}
...
{
    return _engine->_sceneManager
           ? _engine->_sceneManager->setDynamicInstanceVisualTRSWorld(name,
                                                                      WorldVec3(transform.position),
                                                                      transform.rotation,
                                                                      transform.scale)
           : false;
