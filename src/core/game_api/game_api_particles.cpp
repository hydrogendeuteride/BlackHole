...
bool Engine::get_particle_system(uint32_t id, ParticleSystem& out) const
{
    ...
    for (const auto& sys : systems)
    {
        if (sys.id == id)
        {
            ...
            out.params.motionDeltaLocal = p.motion_delta_local;
...
bool Engine::set_particle_system(uint32_t id, const ParticleSystem& system)
{
    ...
    for (auto& sys : systems)
    {
        if (sys.id == id)
        {
            const bool flipbook_changed = sys.flipbook_texture != system.flipbookTexture;
            const bool noise_changed = sys.noise_texture != system.noiseTexture;
            ...
            p.motion_delta_local = system.params.motionDeltaLocal;
            ...
            if (flipbook_changed && !sys.flipbook_texture.empty())
            {
                particlePass->preload_vfx_texture(sys.flipbook_texture);
            }
            if (noise_changed && !sys.noise_texture.empty())
            {
