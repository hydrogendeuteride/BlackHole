...
public:
    ...
    struct PickInfo
    {
        ...
        uint64_t line_segment_primary_id = 0;
        uint64_t line_segment_secondary_id = 0;
        Kind kind = Kind::None;
        SelectionLevel selectionLevel = SelectionLevel::None;
        bool valid = false;
    };

    struct OwnerPickPolicy
    {
        bool hover = true;
        bool click = true;
    };
    ...
    void process_input(const InputSystem &input,
                       ...
                       uint32_t consumed_mouse_button_mask = 0);
    ...
    void clear_drag_selection() { _drag_selection.clear(); }

    // Optional game-owned projection used only for persistent selection visuals.
    // Raw click and marquee results remain available through last_pick() and
    // drag_selection() and are never overwritten by the projection.
    bool selection_projection_active() const { return _selection_projection_active; }
    const std::vector<PickInfo> &selection_projection() const { return _selection_projection; }
    void set_selection_projection(std::vector<PickInfo> picks)
    {
        _selection_projection.swap(picks);
        _selection_projection_active = true;
    }
    ...
    // Legacy helpers for callers that intentionally use raw pick storage as
    // their selection state. Gameplay uses selection_projection() instead.
    void set_primary_pick(const PickInfo &pick) { _last_pick = pick; }
    void clear_primary_pick()
    {
        _last_pick = {};
        _last_pick_object_id = 0;
    }
    void set_drag_selection(std::vector<PickInfo> picks) { _drag_selection.swap(picks); }

    // Live drag rectangle (window pixels) for marquee visualization. Only valid
    // while drag_active() is true.
    bool drag_active() const { return _drag_state.dragging; }
    glm::vec2 drag_start_window() const { return _drag_state.start; }
    glm::vec2 drag_current_window() const { return _drag_state.current; }
    void cancel_drag_interaction() { _drag_state = {}; }
    ...
    void clear_owner_pick_policies();

    void set_owner_pick_policy(RenderObject::OwnerType owner_type,
                               const std::string &owner_name,
                               OwnerPickPolicy policy);
    void clear_owner_pick_policy(RenderObject::OwnerType owner_type, const std::string &owner_name);
    ...
    bool pick_line(const glm::vec2 &window_pos, PickInfo &out_pick, double *out_depth_m = nullptr) const;

    PickInfo *mutable_last_pick() { return &_last_pick; }
    PickInfo *mutable_hover_pick() { return &_hover_pick; }
    void set_manual_hover_pick(const PickInfo &pick);
    void clear_manual_hover_pick();
...
private:
    ...
    enum class PickUse : uint8_t
    {
        Hover,
        Click,
    };

    bool pick_allowed(const PickInfo &pick, PickUse use) const;
    ...
    bool _manual_hover_pick = false;
    std::vector<PickInfo> _drag_selection{};
    std::vector<PickInfo> _selection_projection{};
    bool _selection_projection_active = false;
    ...
    std::unordered_map<std::string, OwnerPickPolicy> _gltf_owner_pick_policies{};
    std::unordered_map<std::string, OwnerPickPolicy> _mesh_owner_pick_policies{};
    std::unordered_map<std::string, OwnerPickPolicy> _line_owner_pick_policies{};
