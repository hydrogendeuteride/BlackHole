...
init(DeviceManager*, framePoolSizes)
  ...
  ├─ create VkSemaphore (swapchain acquire)
...
── per frame ──
  ...
  ... record passes ...
  submit (wait _swapchainSemaphore, signal present semaphore for acquired image, fence _renderFence)
  present (wait same image's present semaphore)
...
Present semaphores belong to `SwapchainManager`, one per swapchain image.
A frame fence does not guarantee that presentation has consumed its semaphore;
reacquiring that image and waiting on its acquire semaphore permits reuse.
