...
{
    // 0 = CSM only, 1 = CSM + RT assist, 2 = RT only.
    // The API enum keeps the historical Clipmap names for compatibility.
    ...
    // Shadow map resolution (square) used for raster CSM cascades and stabilization (texel snapping).
...
struct RocketPlumeSettings
{
    ...
    float sheathRadius = 0.78f;        // normalized plume radius
    float sheathWidth = 0.12f;         // normalized plume radius

    // Shock diamonds (0 disables)
    float shockStrength = 0.0f;
    float shockFrequency = 10.0f;      // cycles along length (approx)
    float shockWidth = 0.08f;          // normalized plume radius
    float shockDecay = 2.5f;           // axial exponential falloff
...
struct PlanetCloudSettings
{
    ...
    // Beer-Lambert extinction coefficient (1/m), independent from shape density.
    float extinction = 1.8e-4f;

    // Cloud lighting tint.
    glm::vec3 color = glm::vec3(0.96f, 0.98f, 1.0f);

    // Coverage threshold (0..1). Higher = emptier.
    float coverage = 0.30f;

    // Bias for the weather texture's type field: 0 = stratus, 0.5 = neutral, 1 = towering.
    float typeBias = 0.45f;
    ...
    // Relative to assets/. BC5: R = weather modulation, G = independent cloud type.
    ...
    float noiseBlend = 0.35f;     // 0..1 weather-field influence from the noise texture
    float detailErode = 0.60f;    // 0..1 strength of height-aware internal erosion

    // Shape evolution periods (seconds). Set to 0 to disable each layer.
    ...
    // Maximum cloud raymarch steps inside the shell (resolution/distance adaptive).
    int cloudSteps = 24;
...
public:
    ...
    glm::vec3 backgroundColor{0.008f};

    bool enableRocketPlumes = false;             // optional analytic rocket plume raymarch pass
    int rocketPlumeSteps = 64;                   // maximum axial raymarch steps for plume rendering
    std::string rocketPlumeNoiseTexturePath{"vfx/simplex.ktx2"}; // relative to assets/
    std::vector<RocketPlumeSettings> rocketPlumes{16}; // initial slots; grows through GameAPI
