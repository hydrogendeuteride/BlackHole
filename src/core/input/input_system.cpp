...
{
    struct SdlEventRecord
    {
        SDL_Event event{};
        std::string extended_edit_utf8{};
    };

    std::vector<SdlEventRecord> sdl_events{};
};
...
void InputState::begin_frame()
{
    ...
    std::fill(_mouse_double_clicked.begin(), _mouse_double_clicked.end(), 0);
...
bool InputState::mouse_double_clicked(MouseButton button) const
{
    const size_t idx = mouse_index(button);
    if (idx >= _mouse_double_clicked.size()) return false;
    return _mouse_double_clicked[idx] != 0;
}
...
void InputState::set_mouse_button(MouseButton button, bool down, const uint8_t click_count)
{
    const size_t idx = mouse_index(button);
...
void InputSystem::begin_frame()
{
    _state.begin_frame();
    _events.clear();
    _text_input.committed_utf8.clear();
    if (_impl)
    {
...
void InputSystem::pump_events()
{
    ...
    while (SDL_PollEvent(&e) != 0)
    {
        ...
        {
            _text_input.composing_utf8.clear();
            _text_input.selection_start = 0;
            _text_input.selection_length = 0;
            _text_input.committed_utf8.append(e.text.text);
        }
        else if (e.type == SDL_TEXTEDITING)
        {
            _text_input.composing_utf8 = e.edit.text;
            _text_input.selection_start = e.edit.start;
            _text_input.selection_length = e.edit.length;
        }
        else if (e.type == SDL_TEXTEDITING_EXT)
        {
            _text_input.composing_utf8 = e.editExt.text ? e.editExt.text : "";
            _text_input.selection_start = e.editExt.start;
            _text_input.selection_length = e.editExt.length;
        }

        Impl::SdlEventRecord native_event{};
        native_event.event = e;
        if (e.type == SDL_TEXTEDITING_EXT)
        {
            native_event.extended_edit_utf8 = e.editExt.text ? e.editExt.text : "";
            native_event.event.editExt.text = nullptr;
            SDL_free(e.editExt.text);
            e.editExt.text = nullptr;
        }
        _impl->sdl_events.push_back(std::move(native_event));
        ...
        else if (e.type == SDL_MOUSEBUTTONDOWN || e.type == SDL_MOUSEBUTTONUP)
        {
            ...
            _state.set_modifiers(mods);
            _state.set_mouse_button(btn, down, e.button.clicks);
            ...
            ev.mods = mods;
            ev.mouse_button = btn;
            ev.click_count = e.button.clicks;
...
void InputSystem::start_text_input()
{
    SDL_StartTextInput();
}

void InputSystem::stop_text_input()
{
    SDL_StopTextInput();
    _text_input.composing_utf8.clear();
    _text_input.selection_start = 0;
...
bool InputSystem::text_input_active() const
{
    return SDL_IsTextInputActive() == SDL_TRUE;
}

void InputSystem::set_text_input_area(const int32_t x,
                                      const int32_t y,
                                      const int32_t width,
                                      const int32_t height)
{
    const SDL_Rect rect{x, y, std::max(width, 1), std::max(height, 1)};
    SDL_SetTextInputRect(&rect);
}

void InputSystem::clear_text_composition()
{
...
void InputSystem::set_mouse_capture(bool enabled)
{
    if (_mouse_captured == enabled)
    {
        return;
    }
...
void InputSystem::for_each_native_event(NativeEventCallback callback, void *user) const
{
    ...
    for (const Impl::SdlEventRecord &record: _impl->sdl_events)
    {
        SDL_Event event = record.event;
        if (event.type == SDL_TEXTEDITING_EXT)
        {
            // NativeEventView is valid only for the duration of the callback.
            event.editExt.text = const_cast<char *>(record.extended_edit_utf8.c_str());
        }
