...
constexpr uint32_t mouse_button_mask(MouseButton button)
{
...
{
    // UTF-8 text committed during the current frame. SDL can split long text
    // across events, so the input system concatenates all pieces in order.
    std::string committed_utf8;

    // UTF-8 text currently being composed by the platform IME. This persists
    // across frames until composition changes, commits, or is cancelled.
    std::string composing_utf8;
    int32_t selection_start = 0;
    int32_t selection_length = 0;

    bool has_composition() const { return !composing_utf8.empty(); }
};
...
public:
    ...
    bool mouse_double_clicked(MouseButton button) const;
...
private:
    ...
    void set_mouse_button(MouseButton button, bool down, uint8_t click_count = 0);
    ...
    std::array<uint8_t, kMouseButtonCount> _mouse_double_clicked{};
...
public:
    ...
    const TextInputState &text_input() const { return _text_input; }

    // Cross-platform SDL text input control. The input area is in logical
    // window coordinates and positions native IME candidate UI where supported.
    ...
    bool mouse_captured() const { return _mouse_captured; }
