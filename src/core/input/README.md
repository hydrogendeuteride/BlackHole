...
lifecycle (quit, minimize, resize), cursor mode switching, and visible mouse capture.
...
| `TextInputState` | Per-frame UTF-8 commits plus persistent platform IME composition state |
...
| `CursorMode` | Cursor visibility/relative mode (Normal, Hidden, Relative) |
...
pump_events()
  └─ SDL_PollEvent loop
       ...
       ├─ collects UTF-8 commits and IME composition from SDL text events
...
set_mouse_capture(enabled)
  └─ keeps mouse events routed to the window during visible drag interactions

text_input()
  └─ exposes committed UTF-8 for this frame and persistent composition state
...
ctx->input->set_mouse_capture(true);                 // visible drag capture
```

### UTF-8 text and IME composition

```cpp
const TextInputState &text = ctx->input->text_input();
value += text.committed_utf8;

// Render composing_utf8 separately, using selection_start/selection_length
// to distinguish the active composition selection when desired.
draw_composition(text.composing_utf8,
                 text.selection_start,
                 text.selection_length);

ctx->input->set_text_input_area(x, y, width, height);
ctx->input->start_text_input();
```

Committed text is cleared by `begin_frame()`; composition persists until it
changes, commits, loses focus, or is explicitly cleared. SDL supplies UTF-8 on
both Windows and Linux. Consumers should still respect ImGui keyboard capture.
