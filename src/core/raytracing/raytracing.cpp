...
void RayTracingManager::flushPendingDeletes()
{
    ...
    Logger::debug("[RT] flushPendingDeletes: destroying {} BLAS handles", _pendingBlasDestroy.size());
    VkDevice dv = _device->device();
...
AccelStructureHandle RayTracingManager::getOrBuildBLAS(const std::shared_ptr<MeshAsset> &mesh)
{
    ...
    {
        Logger::debug("[RT] getOrBuildBLAS reuse by mesh mesh='{}' handle={}", mesh->name,
    ...
    {
        Logger::debug("[RT] getOrBuildBLAS pending build mesh='{}'", mesh->name);
        return {};
    }
    ...
    {
        Logger::debug("[RT] getOrBuildBLAS: flushing pending resource uploads before queuing BLAS build");
        _resources->process_queued_uploads_immediate();
    }

    Logger::debug("[RT] getOrBuildBLAS queue build mesh='{}'", mesh->name);
...
AccelStructureHandle RayTracingManager::buildBlasForMesh(const MeshAsset *mesh)
{
    ...
    {
        Logger::debug("[RT] buildBlasForMesh: flushing pending resource uploads before BLAS build");
        _resources->process_queued_uploads_immediate();
    }
    ...
    Logger::debug("[RT] buildBlasForMesh mesh='{}' surfaces={} vcount={}", mesh->name,
    ...
    if (geoms.empty())
    {
        Logger::debug("[RT] buildBlasForMesh: mesh='{}' has no primitives; skipping BLAS", mesh->name);
    ...
    blas.storage = _resources->create_buffer(blas.size,
    ...
    _resources->immediate_submit([&](VkCommandBuffer cmd) {
        ...
        Logger::debug("[RT] building BLAS for mesh='{}' geoms={} primsTotal={} storageSize={} scratchSize={}",
                      mesh->name,
                      geoms.size(),
...
{
    const VkDeviceSize current_size = _tlas.size;
    ...
    _tlas.size = requiredASSize;
    _tlas.storage = _resources->create_buffer(_tlas.size,
                                              VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR |
                                              VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
                                              VMA_MEMORY_USAGE_GPU_ONLY);
