...
static SDL_Rect usable_display_bounds(int display_index)
{
...
static WindowFrameMargins window_frame_margins(SDL_Window *window)
{
...
static void clamp_window_rect_to_display(SDL_Window *window, int display_index, int &x, int &y, int &w, int &h)
{
    w = std::max(1, w);
    h = std::max(1, h);

    const SDL_Rect bounds = usable_display_bounds(display_index);
...
void VulkanEngine::init()
{
    ...
    // SDL prefers X11 when both backends are available. Prefer native Wayland
    // in a Wayland session so compositor text-input protocols can deliver IME
    // composition, while retaining X11 as a fallback and honoring user choice.
    const char *session_type = SDL_getenv("XDG_SESSION_TYPE");
    const char *wayland_display = SDL_getenv("WAYLAND_DISPLAY");
    if (!SDL_getenv("SDL_VIDEODRIVER") &&
        session_type && SDL_strcasecmp(session_type, "wayland") == 0 &&
    ...
    // Keep SDL text input UTF-8 based and expose IME composition to the
    // application on both Windows and Linux. These must be set before the
    // video subsystem and window are created.
...
void VulkanEngine::setWindowMode(WindowMode mode, int display_index)
{
    ...
    if (mode == WindowMode::Windowed)
    {
        ...
        const bool has_saved_rect = _windowedRect.valid;
        if (has_saved_rect)
        {
            target_x = _windowedRect.x;
            target_y = _windowedRect.y;
        ...
        if (target_w > 0 && target_h > 0)
        {
            ...
            {
                const SDL_Rect bounds = usable_display_bounds(display_index);
                target_x = bounds.x + std::max(0, (bounds.w - target_w) / 2);
                target_y = bounds.y + std::max(0, (bounds.h - target_h) / 2);
            }
            clamp_window_rect_to_display(_window, display_index, target_x, target_y, target_w, target_h);
            SDL_SetWindowSize(_window, target_w, target_h);
            SDL_SetWindowPosition(_window, target_x, target_y);
        }
    }
    else
    {
        // Move the window to the selected display before entering fullscreen.
...
void VulkanEngine::cleanup()
{
    ...
    if (_isInitialized)
    {
        ...
        if (_input)
        {
            _input->set_mouse_capture(false);
...
void VulkanEngine::prepare_scene_for_frame()
{
    if (!_sceneManager)
    {
        return;
    ...
    _scenePreparedForFrame = true;
}

void VulkanEngine::draw()
{
    if (!_scenePreparedForFrame)
    {
        prepare_scene_for_frame();
    }
    _scenePreparedForFrame = false;
    ...
    if (_renderGraph)
    {
        ...
        if (_renderPassManager)
        {
            ...
            if (_context->shadowSettings.enabled && _context->shadowSettings.mode == 2u)
            {
                ...
                // When a planet surface is close enough to fall inside CSM cascades, we render
            ...
            if (auto *atmosphere = _renderPassManager->getPass<AtmospherePass>())
                cloudShadow = atmosphere->register_cloud_shadow(_renderGraph.get());
            if (auto *lighting = _renderPassManager->getPass<LightingPass>())
            {
                lighting->register_graph(_renderGraph.get(), hDraw, hGBufferPosition, hGBufferNormal, hGBufferAlbedo, hGBufferExtra,
                                         ...
                                         std::span<RGImageHandle>(hSpotShadowMaps.data(), hSpotShadowMaps.size()),
                                         std::span<RGImageHandle>(hPointShadowFaces.data(), hPointShadowFaces.size()), cloudShadow);
            }
            ...
            if (auto *ocean = _renderPassManager->getPass<OceanPass>())
            {
                ...
                ocean->register_graph(_renderGraph.get(), hdrTarget, hDepth, hGBufferPosition, {}, cloudShadow);
            }
    ...
    VkSemaphoreSubmitInfo waitInfo = vkinit::semaphore_submit_info(VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR,
                                                                   get_current_frame()._swapchainSemaphore);
    const VkSemaphore presentSemaphore = _swapchainManager->present_semaphore(swapchainImageIndex);
    VkSemaphoreSubmitInfo signalInfo = vkinit::semaphore_submit_info(VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
                                                                     presentSemaphore);
...
void VulkanEngine::run()
{
    ...
    while (!bQuit)
    {
        ...
        const bool ui_capture_mouse = _ui && _ui->wantCaptureMouse();
        bool ui_capture_keyboard = _ui && _ui->wantCaptureKeyboard();
        ...
        {
            if (_sceneManager->getCameraRig().mode() == CameraMode::Free &&
                _input->state().mouse_pressed(MouseButton::Right) && !ui_capture_mouse &&
                _ui && _ui->releaseKeyboardFocus())
            {
                ui_capture_keyboard = false;
            }
