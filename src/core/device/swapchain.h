...
public:
    ...
    VkSemaphore present_semaphore(uint32_t index) const { return _presentSemaphores.at(index); }
...
private:
    ...
    // Presentation may outlive a frame fence. Reuse only after acquiring the same image.
