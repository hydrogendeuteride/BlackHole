...
void SwapchainManager::create_swapchain(uint32_t width, uint32_t height)
{
    ...
    _presentSemaphores.assign(_swapchainImages.size(), VK_NULL_HANDLE);
    const VkSemaphoreCreateInfo semaphoreInfo = vkinit::semaphore_create_info();
    for (VkSemaphore &semaphore : _presentSemaphores)
    {
        VK_CHECK(vkCreateSemaphore(_deviceManager->device(), &semaphoreInfo, nullptr, &semaphore));
    }
}
...
void SwapchainManager::destroy_swapchain() const
{
    for (VkSemaphore semaphore : _presentSemaphores)
    {
        vkDestroySemaphore(_deviceManager->device(), semaphore, nullptr);
    }
...
void SwapchainManager::resize_swapchain(struct SDL_Window *window)
{
    ...
    // OUT_OF_DATE/SUBOPTIMAL can require recreation even at the same extent.
