...
namespace vk_engine::debug_ui
{
    void ui_postfx(VulkanEngine *eng)
    {
        ...
        for (size_t i = 0; i < ctx->rocketPlumes.size(); ++i)
        {
            ...
            ImGui::SliderFloat(("Sheath Strength" + id).c_str(), &ps.sheathStrength, 0.0f, 3.0f);
            ImGui::SliderFloat(("Sheath Radius" + id).c_str(), &ps.sheathRadius, 0.0f, 1.0f);
            ImGui::SliderFloat(("Sheath Width" + id).c_str(), &ps.sheathWidth, 0.01f, 0.5f);
            ...
            ImGui::SliderFloat(("Shock Width" + id).c_str(), &ps.shockWidth, 0.005f, 0.5f);
            ImGui::SliderFloat(("Shock Decay" + id).c_str(), &ps.shockDecay, 0.0f, 12.0f);
        ...
        ImGui::TextUnformatted("Overlay drives coverage; weather R shapes breakup and G (or legacy R) selects cloud type.");
        ...
        float extinctionPerKm = clouds.extinction * 1000.0f;
        if (ImGui::SliderFloat("Extinction (/km)", &extinctionPerKm, 0.01f, 2.0f, "%.3f"))
        {
            clouds.extinction = std::max(0.0f, extinctionPerKm * 1.0e-3f);
        }
        ...
        ImGui::SliderFloat("Cloud Type Bias", &clouds.typeBias, 0.0f, 1.0f, "%.2f");
        clouds.typeBias = std::clamp(clouds.typeBias, 0.0f, 1.0f);
        ImGui::TextUnformatted("0 = More stratus, 0.5 = Mixed, 1 = More towering");
        ...
        float weatherEvolutionMin = clouds.weatherEvolutionPeriodS / 60.0f;
        if (ImGui::SliderFloat("Weather Evolution (min)", &weatherEvolutionMin, 0.0f, 120.0f, "%.1f"))
        {
            clouds.weatherEvolutionPeriodS = std::max(0.0f, weatherEvolutionMin * 60.0f);
        }
        float detailEvolutionMin = clouds.detailEvolutionPeriodS / 60.0f;
        if (ImGui::SliderFloat("Detail Evolution (min)", &detailEvolutionMin, 0.0f, 60.0f, "%.1f"))
        {
            clouds.detailEvolutionPeriodS = std::max(0.0f, detailEvolutionMin * 60.0f);
        }
        ImGui::TextUnformatted("Evolution 0 disables that layer; wind moves the whole cloud field.");
        ...
        ImGui::Checkbox("Cloud Shadows", &clouds.shadowsEnabled);
        if (clouds.shadowsEnabled)
        {
            ImGui::SliderFloat("Cloud Shadow Strength", &clouds.shadowStrength, 0.0f, 2.0f);
            ImGui::TextUnformatted("Strength 1 is neutral; above 1 deepens cloud shadows.");
            float shadowRangeKm = clouds.shadowExtentM * 0.001f;
            if (ImGui::SliderFloat("Near Shadow Range (km)", &shadowRangeKm, 8.0f, 2048.0f, "%.0f"))
                clouds.shadowExtentM = shadowRangeKm * 1000.0f;
            ImGui::TextUnformatted("One local shadow map; shadows fade outside its range.");
            ImGui::SliderInt("Cloud Shadow Steps", &clouds.shadowSteps, 4, 16);
        }

        ImGui::SliderInt("Cloud Steps", &clouds.cloudSteps, 4, 128);
        clouds.cloudSteps = std::clamp(clouds.cloudSteps, 4, 128);

        const char *cloudResItems[] = {"Full", "Half", "Quarter"};
        int cloudResIndex = (clouds.resolutionDiv >= 4) ? 2 : ((clouds.resolutionDiv >= 2) ? 1 : 0);
        if (ImGui::Combo("Cloud Resolution", &cloudResIndex, cloudResItems, 3))
        {
            clouds.resolutionDiv = (cloudResIndex == 2) ? 4 : ((cloudResIndex == 1) ? 2 : 1);
        }
