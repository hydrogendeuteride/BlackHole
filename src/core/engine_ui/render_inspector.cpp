...
namespace vk_engine::debug_ui
{
    void ui_shadows(VulkanEngine *eng)
    {
        ...
        ImGui::RadioButton("CSM only", &mode, 0);
        ImGui::SameLine();
        ImGui::RadioButton("CSM + RT", &mode, 1);
        ImGui::SameLine();
        ...
        ImGui::Separator();
        ImGui::TextWrapped(
            "CSM only: raster PCF+RPDB. CSM+RT: PCF assisted by ray query at low N·L. RT only: skip shadow maps and use ray tests only.");
    }
