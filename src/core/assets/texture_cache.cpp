...
void TextureCache::cleanup()
{
    ...
    for (TextureHandle h = 0; h < _entries.size(); ++h)
    {
        ...
        {
            Logger::debug("[TextureCache] cleanup destroy handle={} path='{}' bytes={}",
                          h,
...
TextureCache::TextureHandle TextureCache::request(const TextureKey &key, VkSampler sampler)
{
    ...
    Logger::debug("[TextureCache] request handle={} kind={} path='{}' srgb={} mipmapped={} hash=0x{:016x}",
                  h,
...
void TextureCache::evictToBudget(size_t budgetBytes)
{
    ...
    for (auto &pair : order)
    {
        ...
        Logger::debug("[TextureCache] evictToBudget destroy handle={} path='{}' bytes={} residentBytesBefore={}",
                      h,
                      e.path.empty() ? "<bytes>" : e.path,
                      e.sizeBytes,
                      _residentBytes);
...
bool TextureCache::unload(TextureHandle handle, bool drop_source_bytes)
{
    ...
    if (e.state == EntryState::Resident && e.image.image != VK_NULL_HANDLE)
    {
        ...
        {
            Logger::debug("[TextureCache] unload destroy handle={} path='{}' bytes={} residentBytesBefore={}",
                          handle,
                          e.path.empty() ? "<bytes>" : e.path,
                          e.sizeBytes,
                          _residentBytes);
...
size_t TextureCache::drainReadyUploads(ResourceManager &rm, size_t budgetBytes)
{
    ...
    for (auto &res : local)
    {
        ...
        if (res.isKTX2)
        {
            ...
            {
                fmt = reqFmt;
                Logger::debug("[TextureCache] Overriding KTX2 format to {} based on request (original {})",
        ...
        if (res.isKTX2)
        {
            ...
            else
            {
                ...
                Logger::debug("[TextureCache] upload KTX2 handle={} fmt={} levels={} size={}x{} srgb={} path='{}'",
                              res.handle,
                              string_VkFormat(fmt),
                              res.ktxMipLevels,
                              extent.width,
        ...
        else
        {
            ...
            Logger::debug("[TextureCache] upload raster handle={} fmt={} levels={} size={}x{} srgb={} path='{}'",
                          res.handle,
                          string_VkFormat(fmt),
                          mipOverride,
                          extent.width,
...
bool TextureCache::tryMakeSpace(size_t bytesNeeded, uint32_t now)
{
    ...
    for (auto &pair : order)
    {
        ...
        patchToFallback(e);
        Logger::debug("[TextureCache] tryMakeSpace destroy handle={} path='{}' bytes={} residentBytesBefore={}",
                      h,
                      e.path.empty() ? "<bytes>" : e.path,
                      e.sizeBytes,
                      _residentBytes);
