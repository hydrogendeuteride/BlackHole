...
{
    float baseHeightM{1500.0f};
    float thicknessM{10000.0f};

    float densityScale{0.85f};
    float extinction{1.8e-4f};
    glm::vec3 color{0.96f, 0.98f, 1.0f};
    float coverage{0.30f};
    float typeBias{0.45f};      // Biases the type map toward stratus or towering clouds

    // Macro cloud/weather map wrapped over the sphere.
    std::string overlayTexturePath{"planets/earth/cloud/fair_clouds_8k_coverage_bc4.ktx2"};
    // BC5 weather map: R = modulation, G = independent cloud type.
    ...
    bool overlayFlipV{true};

    float noiseScale{1.0f};     // macro weather frequency
    float detailScale{12.0f};   // frequency multiplier; 12 maps to 480/120/40 km feature bands
    float noiseBlend{0.35f};    // weather-field influence from the noise texture
    float detailErode{0.60f};   // height-aware internal erosion strength

    float weatherEvolutionPeriodS{1800.0f};
    float detailEvolutionPeriodS{480.0f};

    float windSpeed{12.0f};
    float windAngleRad{glm::radians(25.0f)};

    int cloudSteps{24}; // maximum; scaled by resolution and distance
    // Cloud buffer resolution divisor: 1 = full, 2 = half, 4 = quarter.
    int resolutionDiv{2};

    bool shadowsEnabled = false;
    float shadowStrength = 1.25f; // 1 = original attenuation; above 1 emphasizes shadows
    float shadowExtentM = 131072.0f; // half extent of the local shadow map
    int shadowSteps = 8;
};
...
struct RocketPlumeSettings
{
    ...
    // Kept in double precision until the renderer applies the floating origin.
    glm::dmat4 worldToPlume{1.0};
    ...
    float sheathStrength{0.35f};
    float sheathRadius{0.78f};
    float sheathWidth{0.12f};

    float shockStrength{0.0f};
    float shockFrequency{10.0f};
    float shockWidth{0.08f};
    float shockDecay{2.5f};
...
struct ParticleParams
{
    ...
    // Render-local displacement applied to live particles this frame;
    // consumed once by the update pass, then cleared.
    glm::vec3 motionDeltaLocal{0.0f};
...
struct WindowSettings
{
    ...
    int width{1280};
    int height{720};
    uint32_t logical_width{1920};
    uint32_t logical_height{1080};
...
public:
    ...
    // Shadow map resolution (square). Affects raster CSM cascades + stabilization.
    ...
    // Override the visible background with a linear RGB color; retains IBL lighting.
    void set_plain_background(bool enabled, const glm::vec3& color);
    bool get_plain_background(glm::vec3& color) const;
    ...
    bool set_gltf_instance_visual_rotation(const std::string& name, const glm::quat& rotation);
    ...
    WindowSettings get_window_settings() const;
    void apply_window_settings(const WindowSettings& settings);
    std::vector<DisplayInfo> get_displays() const;
    std::vector<DisplayResolution> get_display_resolutions(int display_index) const;

    void set_logical_render_extent(uint32_t width, uint32_t height);

    void set_render_scale(float scale); // 0.1 - 4.0
    ...
    // Grows the slot list as needed; intermediate slots start disabled.
    bool set_rocket_plume(size_t index, const RocketPlumeSettings& settings);

    // Allocated slots, including disabled ones; not a fixed limit.
    size_t get_rocket_plume_count() const;
