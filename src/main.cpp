...
namespace
{
    ...
    {
        EntryMode mode{EntryMode::Sc};
        std::string log_output{"both"};
        std::string log_level{"info"};
    };

    void set_entry_mode(StartupOptions &options, const std::string &value)
    {
        if (value == "legacy")
        {
            options.mode = EntryMode::Legacy;
        }
        else if (value == "sc")
        {
            options.mode = EntryMode::Sc;
        }
    }
    ...
    StartupOptions parse_startup_options(int argc, char *argv[])
    {
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            if (value.rfind(mode_prefix.data(), 0) == 0)
            {
                set_entry_mode(options, value.substr(mode_prefix.size()));
            }
            else if (value.rfind(log_prefix.data(), 0) == 0)
            {
                options.log_output = value.substr(log_prefix.size());
            }
            else if (value.rfind(loglevel_prefix.data(), 0) == 0)
            {
    ...
    void append_crash_backtrace()
    {
        ...
        {
            append_crash_log(fmt::format("  #{:02d} address={}", i, frames[i]));
        }
    }
#endif
    ...
    std::filesystem::path exe_dir(int argc, char *argv[])
    {
#if defined(_WIN32)
        char module_path[MAX_PATH]{};
        const DWORD size = GetModuleFileNameA(nullptr, module_path, static_cast<DWORD>(sizeof(module_path)));
        ...
        {
            std::error_code ec;
            const auto exe_path = std::filesystem::absolute(argv[0], ec);
            if (!ec)
            {
    ...
    void set_project_work_dir(int argc, char *argv[])
    {
        const auto dir = exe_dir(argc, argv);
        if (dir.empty())
        {
            return;
        }

        const AssetPaths paths = AssetPaths::detect(dir);
        if (paths.root.empty())
        {
            return;
        }
...
{
    set_project_work_dir(argc, argv);
