...
bool SceneManager::setDynamicInstanceVisualTransform(const std::string &name, const glm::mat4 &transform)
{
    return setMeshInstanceTransform(name, transform) || setGLTFInstanceTransform(name, transform, false);
}

bool SceneManager::setDynamicInstanceVisualTRSWorld(const std::string &name,
                                                    const WorldVec3 &translationWorld,
                                                    const glm::quat &rotation,
                                                    const glm::vec3 &scale)
{
    return setMeshInstanceTRSWorld(name, translationWorld, rotation, scale) ||
           setGLTFInstanceTRSWorld(name, translationWorld, rotation, scale, false);
}
...
bool SceneManager::setGLTFInstanceVisualRotation(const std::string &name, const glm::quat &rotation)
{
    auto it = dynamicGLTFInstances.find(name);
    if (it == dynamicGLTFInstances.end())
    {
    ...
    const glm::quat next = glm::normalize(rotation);
    if (std::abs(glm::dot(it->second.visual_rotation, next)) > 0.999999f)
    {
        return true;
    }

    it->second.visual_rotation = next;
    if (auto body_it = _dynamicRootColliderBodies.find(name); body_it != _dynamicRootColliderBodies.end())
    {
        return rebuildDynamicRootColliderBody(name, body_it->second, true);
    }
    return true;
}
