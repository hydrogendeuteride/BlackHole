...
- `planet_patch_helpers.h`
  Internal helpers in `planet_helpers` namespace shared across the `.cpp` files: `compute_patch_bounds`, `make_planet_constants` (PBR material constants with emission and force-CSM receiver flag), `debug_color_for_level`, `recompute_patch_normals`, `refine_patch_edge_normals_from_height`, `reinforce_patch_skirts`, `stitch_patch_edges_to_parent_grid`, `compute_patch_edge_stitch_mask`, `patch_needs_balance_split`.
