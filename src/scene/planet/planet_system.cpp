...
bool PlanetSystem::destroy_planet(std::string_view name)
{
    ...
    if (it->terrain && _context && _context->assets)
    {
        _context->assets->removeMaterialBuffer(fmt::format("Planet_{}_TerrainMaterial", it->name),
                _context->currentFrame ? &_context->currentFrame->_deletionQueue : nullptr);
    }
...
void PlanetSystem::clear_planets(bool destroy_mesh_assets)
{
    if (destroy_mesh_assets && _context && _context->assets)
    {
        ...
        for (const PlanetBody &b: _bodies)
        {
            if (b.terrain)
            {
                assets->removeMaterialBuffer(fmt::format("Planet_{}_TerrainMaterial", b.name),
                        _context->currentFrame ? &_context->currentFrame->_deletionQueue : nullptr);
            }
            if (!b.mesh) continue;
            if (_context->currentFrame)
