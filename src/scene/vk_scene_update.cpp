...
namespace
{
    ...
    constexpr float kCameraNearZ = 0.1f;
    constexpr float kShadowSplitLambda = 0.93f;
...
void SceneManager::emitDynamicGLTFInstances(const WorldVec3 &origin_world, float dt)
{
    ...
    for (auto &kv : dynamicGLTFInstances)
    {
        ...
        const glm::mat4 instance_transform =
                glm::translate(glm::mat4(1.0f), t_local) *
                glm::mat4_cast(inst.rotation) *
                glm::mat4_cast(inst.visual_rotation) *
...
void SceneManager::updateViewProjectionData()
{
    ...
    glm::mat4 projection = make_reversed_infinite_perspective(fov, aspect, kCameraNearZ);
    projection[1][1] *= -1.0f;
...
void SceneManager::updateDirectionalShadowData()
{
    ...
    const glm::vec3 cam_right = glm::normalize(glm::vec3(cam_rot[0]));
    const glm::vec3 cam_up = glm::normalize(glm::vec3(cam_rot[1]));
    ...
    const float aspect = static_cast<float>(logical_extent.width) /
                         static_cast<float>(logical_extent.height);
    const float tan_half_fov = std::tan(glm::radians(mainCamera.fovDegrees) * 0.5f);
    const float shadow_far = std::max(kShadowCSMFar, kCameraNearZ + 1.0f);

    std::array<float, kShadowCascadeCount> splits{};
    for (int ci = 0; ci < kShadowCascadeCount; ++ci)
    {
        const float p = static_cast<float>(ci + 1) / static_cast<float>(kShadowCascadeCount);
        const float log_split = kCameraNearZ * std::pow(shadow_far / kCameraNearZ, p);
        const float linear_split = kCameraNearZ + (shadow_far - kCameraNearZ) * p;
        splits[ci] = glm::mix(linear_split, log_split, kShadowSplitLambda);
    }
    ...
    for (int ci = 0; ci < kShadowCascadeCount; ++ci)
    {
        const float prev_split = (ci == 0) ? kCameraNearZ : splits[ci - 1];
        const float next_split = splits[ci];
        const float split_pad = std::max(0.5f, (next_split - prev_split) * 0.05f);
        const float slice_near = std::max(kCameraNearZ, prev_split - split_pad);
        const float slice_far = std::min(shadow_far, next_split + split_pad);

        std::array<glm::vec3, 8> corners{};
        int corner_i = 0;
        for (float d : {slice_near, slice_far})
        {
            const float y = tan_half_fov * d;
            const float x = y * aspect;
            const glm::vec3 center = cam_pos + cam_fwd * d;
            corners[corner_i++] = center - cam_right * x - cam_up * y;
            corners[corner_i++] = center + cam_right * x - cam_up * y;
            corners[corner_i++] = center - cam_right * x + cam_up * y;
            corners[corner_i++] = center + cam_right * x + cam_up * y;
        }

        glm::vec3 center_target(0.0f);
        for (const glm::vec3 &corner : corners)
        {
            center_target += corner;
        }
        center_target /= static_cast<float>(corners.size());

        float radius = 0.0f;
        for (const glm::vec3 &corner : corners)
        {
            radius = std::max(radius, glm::length(corner - center_target));
        }
        radius = std::max(radius, level_radius(0));
        ...
        const float pullback = glm::max(kShadowClipPullbackMin,
                                        radius + cover * kShadowClipPullbackFactor);
        const glm::vec3 eye = center + light_dir * pullback;
        const glm::mat4 view = glm::lookAtRH(eye, center, up);
        const float z_far = pullback + radius + cover * kShadowClipForwardFactor + kShadowClipZPadding;
