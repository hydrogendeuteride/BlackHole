	...
	add_pass(
		...
		[sourceDraw, targetSwapchain](VkCommandBuffer cmd, const RGPassResources &res, EngineContext *ctx) {
			...
			if (!ctx->pipelines->getGraphics("present_letterbox", pipeline, layout))
			{
				...
				info.vertexShaderPath = ctx->getAssets()->shaderPath("common/fullscreen.vert.spv");
				info.fragmentShaderPath = ctx->getAssets()->shaderPath("post/present_letterbox.frag.spv");
