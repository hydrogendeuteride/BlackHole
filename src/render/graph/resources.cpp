...
void RGResourceRegistry::reset()
{
    ...
    _imageLookup.clear();
    _bufferLookup.clear();
    _imageUseCounts.clear();
    _bufferUseCounts.clear();
}

void RGResourceRegistry::shutdown()
{
    if (_ctx && _ctx->getResources())
    {
        ResourceManager *resources = _ctx->getResources();
        for (auto &[frame, cache] : _frameCaches)
        {
            (void)frame;
            for (auto &[name, entries] : cache.images)
            {
                (void)name;
                for (const CachedImage &entry : entries)
                {
                    if (entry.allocation.image != VK_NULL_HANDLE)
                    {
                        resources->destroy_image(entry.allocation);
                    }
                }
            }
            for (auto &[name, entries] : cache.buffers)
            {
                (void)name;
                for (const CachedBuffer &entry : entries)
                {
                    if (entry.allocation.buffer != VK_NULL_HANDLE)
                    {
                        resources->destroy_buffer(entry.allocation);
                    }
                }
            }
        }
...
RGImageHandle RGResourceRegistry::add_transient(const RGImageDesc& d)
{
    ...
    rec.format = d.format;
    rec.extent = d.extent;
    rec.depth = d.depth > 0 ? d.depth : 1;
    rec.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
	rec.initialStage = VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT;
	rec.initialAccess = 0;
    rec.creationUsage = d.usage;

    VkExtent3D size{d.extent.width, d.extent.height, rec.depth};
    if (_ctx && _ctx->currentFrame)
    {
        const size_t use_index = _imageUseCounts[d.name]++;
        auto &entries = _frameCaches[_ctx->currentFrame].images[d.name];
        if (entries.size() <= use_index)
        {
            CachedImage entry{};
            entry.format = d.format;
            entry.extent = d.extent;
            entry.depth = rec.depth;
            entry.usage = d.usage;
            entry.allocation = _ctx->getResources()->create_image(size, d.format, d.usage);
            entries.push_back(entry);
        }
        else
        {
            CachedImage &entry = entries[use_index];
            const bool matches = entry.format == d.format &&
                                 entry.extent.width == d.extent.width &&
                                 entry.extent.height == d.extent.height &&
                                 entry.depth == rec.depth &&
                                 entry.usage == d.usage;
            if (!matches)
            {
                _ctx->getResources()->destroy_image(entry.allocation);
                entry.format = d.format;
                entry.extent = d.extent;
                entry.depth = rec.depth;
                entry.usage = d.usage;
        ...
        rec.allocation = entries[use_index].allocation;
    }
    else
    {
...
RGBufferHandle RGResourceRegistry::add_transient(const RGBufferDesc& d)
{
	...
	if (_ctx && _ctx->currentFrame)
	{
		const size_t use_index = _bufferUseCounts[d.name]++;
		auto &entries = _frameCaches[_ctx->currentFrame].buffers[d.name];
		if (entries.size() <= use_index)
		{
			CachedBuffer entry{};
			entry.size = d.size;
			entry.usage = d.usage;
			entry.memoryUsage = d.memoryUsage;
			entry.allocation = _ctx->getResources()->create_buffer(d.size, d.usage, d.memoryUsage);
			entries.push_back(entry);
		}
		else
		{
			CachedBuffer &entry = entries[use_index];
			const bool matches = entry.size == d.size &&
			                     entry.usage == d.usage &&
			                     entry.memoryUsage == d.memoryUsage;
			if (!matches)
			{
				_ctx->getResources()->destroy_buffer(entry.allocation);
				entry.size = d.size;
				entry.usage = d.usage;
				entry.memoryUsage = d.memoryUsage;
				entry.allocation = _ctx->getResources()->create_buffer(d.size, d.usage, d.memoryUsage);
