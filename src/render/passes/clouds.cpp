...
void CloudPass::init(EngineContext *context)
{
    ...
    info.fragmentShaderPath = _context->getAssets()->shaderPath("effects/clouds.frag.spv");
    info.setLayouts = {
    ...
    // Optional voxel advection compute pipeline (used when VoxelVolumeSettings::animateVoxels is enabled).
    {
        ...
        ci.shaderPath = _context->getAssets()->shaderPath("effects/cloud_voxel_advect.comp.spv");
