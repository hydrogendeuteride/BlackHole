...
void OceanPass::init(EngineContext *context)
{
    ...
    atmosphereBuilder.add_binding(1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    atmosphereBuilder.add_binding(3, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
    ...
    info.vertexShaderPath = _context->getAssets()->shaderPath("planet/ocean.vert.spv");
    info.fragmentShaderPath = _context->getAssets()->shaderPath("planet/ocean.frag.spv");
    info.setLayouts = {
                               ...
                               CloudShadowMap cloudShadow)
{
    ...
    graph->add_pass(
        "Ocean",
        RGPassType::Graphics,
        [drawHandle, depthHandle, surfacePositionHandle, transmittanceLut, cloudShadow](RGPassBuilder &builder, EngineContext *ctx)
        {
            if (cloudShadow.image.valid()) builder.read(cloudShadow.image, RGImageUsage::SampledFragment);
        ...
        [this, drawHandle, depthHandle, surfacePositionHandle, transmittanceLut, cloudShadow](
            VkCommandBuffer cmd,
            const RGPassResources &resources,
            EngineContext *ctx)
        {
            draw_ocean(cmd, ctx, resources, drawHandle, depthHandle, surfacePositionHandle, transmittanceLut, cloudShadow);
        });
}
...
void OceanPass::draw_ocean(VkCommandBuffer cmd,
                           ...
                           RGImageHandle,
                           RGImageHandle transmittanceLut,
                           CloudShadowMap cloudShadow) const
{
    ...
    VkDescriptorSet atmosphere_set = VK_NULL_HANDLE;
    {
        ...
        atmosphere_writer.write_image(0,
                                      ...
                                      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                      VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
        const std::array<RGImageHandle, 1> maps{cloudShadow.image};
        for (uint32_t i = 0; i < maps.size(); ++i)
        {
            VkImageView view = maps[i].valid() ? resources.image_view(maps[i]) : ctx_local->getAssets()->fallbackWhiteView();
            atmosphere_writer.write_image(1 + i, view, ctx_local->getSamplers()->linearClampEdge(),
                VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
        }
        AllocatedBuffer shadow_buffer = resource_manager->create_buffer(sizeof(CloudShadowData),
            VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
        std::memcpy(shadow_buffer.info.pMappedData, &cloudShadow.data, sizeof(CloudShadowData));
        vmaFlushAllocation(device_manager->allocator(), shadow_buffer.allocation, 0, sizeof(CloudShadowData));
        ctx_local->currentFrame->_deletionQueue.push_function([resource_manager, shadow_buffer]()
        {
            resource_manager->destroy_buffer(shadow_buffer);
        });
        atmosphere_writer.write_buffer(3, shadow_buffer.buffer, sizeof(CloudShadowData), 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
    ...
    const float atmosphere_radius = (ctx_local->enableAtmosphere && atmosphere_planet_radius > 0.0f && atmosphere_height > 0.0f)
        ...
        : 0.0f;
    const glm::vec3 beta_rayleigh = (ctx_local->enableAtmosphere && MathUtil::finite(atmosphere.rayleighScattering))
        ? glm::max(atmosphere.rayleighScattering, glm::vec3(0.0f))
        : glm::vec3(0.0f);
    const glm::vec3 beta_mie = (ctx_local->enableAtmosphere && MathUtil::finite(atmosphere.mieScattering))
        ? glm::max(atmosphere.mieScattering, glm::vec3(0.0f))
        : glm::vec3(0.0f);
    const glm::vec3 absorption_color = MathUtil::finite(atmosphere.absorptionColor)
