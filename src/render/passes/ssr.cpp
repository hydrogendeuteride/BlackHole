...
void SSRPass::init(EngineContext *context)
{
    ...
    infoNoRT.fragmentShaderPath = _context->getAssets()->shaderPath("post/ssr.frag.spv");
    ...
    if (wantRTAtStartup)
    {
        ...
        infoRT.fragmentShaderPath = _context->getAssets()->shaderPath("post/ssr_rt.frag.spv");
    ...
    if (useRT)
    {
        ...
        if (!pipelineManager->getGraphics("ssr.rt", p, l))
        {
            if (!_rtPipelineCreateAttempted)
            {
                ...
                GraphicsPipelineCreateInfo infoRT = baseInfo;
                infoRT.fragmentShaderPath = ctxLocal->getAssets()->shaderPath("post/ssr_rt.frag.spv");
