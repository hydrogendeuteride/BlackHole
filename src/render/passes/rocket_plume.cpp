...
namespace
{
    struct GPURocketPlume
    {
        ...
        glm::vec4 shock_misc;     // x shockFrequency, y shockWidth, z shockDecay, w unused
        glm::vec4 sheath;         // x strength, y radius, z width, w unused
    };
...
void RocketPlumePass::update_noise_texture(EngineContext *ctxLocal)
{
    ...
    std::error_code ec;
    if (key.path.empty() || !std::filesystem::is_regular_file(key.path, ec) || ec)
    {
        _noisePath = desired;
        return;
    }
...
void RocketPlumePass::init(EngineContext *context)
{
    ...
    info.fragmentShaderPath = _context->getAssets()->shaderPath("effects/rocket_plume.frag.spv");
    info.setLayouts = {
...
RGImageHandle RocketPlumePass::register_graph(RenderGraph *graph, RGImageHandle hdrInput, RGImageHandle gbufPos)
{
    ...
    instances.reserve(ctxLocal->rocketPlumes.size());
    for (const RocketPlumeSettings &ps : ctxLocal->rocketPlumes)
    {
        if (!ps.enabled)
        {
            continue;
        ...
        GPURocketPlume gpu{};
        glm::dmat4 local_to_plume = ps.worldToPlume;
        local_to_plume[3] += ps.worldToPlume *
                glm::dvec4(ctxLocal->origin_world, 0.0);
        gpu.world_to_plume = glm::mat4(local_to_plume);
        ...
        gpu.shock_misc = glm::vec4(std::max(0.0f, ps.shockFrequency),
                                   std::max(0.001f, ps.shockWidth),
                                   std::max(0.0f, ps.shockDecay),
                                   0.0f);
        gpu.sheath = glm::vec4(std::max(0.0f, ps.sheathStrength),
                               std::clamp(ps.sheathRadius, 0.0f, 1.0f),
                               std::max(0.001f, ps.sheathWidth),
                               0.0f);
                                  ...
                                  uint32_t plumeCount)
{
    ...
    if (!pipelineManager->getGraphics(
            "rocket_plume", _pipeline, _pipelineLayout))
    {
        return;
    }
