...
void LightingPass::init(EngineContext *context)
{
    ...
    baseInfo.vertexShaderPath = _context->getAssets()->shaderPath("common/fullscreen.vert.spv");
    baseInfo.setLayouts.assign(std::begin(layouts), std::end(layouts));
    baseInfo.pushConstants = {{VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(CloudShadowData)}};
    ...
    infoNoRT.fragmentShaderPath = _context->getAssets()->shaderPath("lighting/deferred_lighting_nort.frag.spv");
    _context->pipelines->createGraphicsPipeline("deferred_lighting.nort", infoNoRT);
	    ...
	    if (wantRTAtStartup)
	    {
	        ...
	        infoRT.fragmentShaderPath = _context->getAssets()->shaderPath("lighting/deferred_lighting.frag.spv");
	    ...
	    else
	    {
	        ...
	        {
	            Logger::info("[LightingPass] RT pipeline deferred (shadow mode = CSM only).");
	        }
                                  ...
                                  CloudShadowMap cloudShadow)
{
    ...
    graph->add_pass(
        "Lighting",
        RGPassType::Graphics,
        [drawHandle, gbufferPosition, gbufferNormal, gbufferAlbedo, gbufferExtra, shadowCascades, spotShadowMaps, pointShadowFaces, cloudShadow](RGPassBuilder &builder, EngineContext *)
        {
        ...
        [this, drawHandle, gbufferPosition, gbufferNormal, gbufferAlbedo, gbufferExtra, shadowCascades, spotShadowMaps, pointShadowFaces, cloudShadow](VkCommandBuffer cmd, const RGPassResources &res, EngineContext *ctx)
        {
            draw_lighting(cmd, ctx, res, drawHandle, gbufferPosition, gbufferNormal, gbufferAlbedo, gbufferExtra,
                          shadowCascades, spotShadowMaps, pointShadowFaces, cloudShadow);
        });
}
	    ...
	    if (useRT)
	    {
	        ...
	        if (!pipelineManager->getGraphics("deferred_lighting.rt", p, l))
	        {
	            if (!_rtPipelineCreateAttempted)
	            {
	                ...
	                GraphicsPipelineCreateInfo baseInfo{};
	                baseInfo.vertexShaderPath = ctxLocal->getAssets()->shaderPath("common/fullscreen.vert.spv");
	                baseInfo.setLayouts.assign(std::begin(layouts), std::end(layouts));
                    baseInfo.pushConstants = {{VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(CloudShadowData)}};
	                ...
	                auto infoRT = baseInfo;
	                infoRT.fragmentShaderPath = ctxLocal->getAssets()->shaderPath("lighting/deferred_lighting.frag.spv");
    ...
    *sceneUniformData = ctxLocal->getSceneData();
        ...
        deviceManager->device(), _gBufferInputDescriptorLayout);
    {
        ...
        gbw.write_image(3, extView, ctxLocal->getSamplers()->defaultLinear(),
                        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
        VkImageView cloudView = cloudShadow.image.valid() ? resources.image_view(cloudShadow.image)
                                                        : ctxLocal->getAssets()->fallbackWhiteView();
        gbw.write_image(4, cloudView, ctxLocal->getSamplers()->linearClampEdge(),
                        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
        gbw.update_set(deviceManager->device(), gbufferSet);
    }

    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, _pipeline);
    vkCmdPushConstants(cmd, _pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                       sizeof(CloudShadowData), &cloudShadow.data);
