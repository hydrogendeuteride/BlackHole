...
namespace
{
    ...
    struct ParticleUpdatePushConstants
    {
        ...
        glm::vec4 origin_delta;         // xyz origin delta local, w=motion delta z
        ...
        glm::vec4 size_range;           // x=minSize, y=maxSize, z/w=motion delta x/y
        glm::vec4 color;                // rgba
    };
...
void ParticlePass::init(EngineContext *context)
{
    ...
    // Compute update pipeline + instance.
    {
        ...
        ci.shaderPath = _context->getAssets()->shaderPath("particles/particles_update.comp.spv");
    ...
    // Compute block sort + index build pipelines (alpha block-sorting).
    {
        ...
        sortCi.shaderPath = _context->getAssets()->shaderPath("particles/particles_sort_blocks.comp.spv");
        ...
        buildCi.shaderPath = _context->getAssets()->shaderPath("particles/particles_build_indices.comp.spv");
    ...
    {
        const std::string vert = _context->getAssets()->shaderPath("particles/particles.vert.spv");
        const std::string frag = _context->getAssets()->shaderPath("particles/particles.frag.spv");
    ...
    graph->add_pass(
        ...
        [this](VkCommandBuffer cmd, const RGPassResources &, EngineContext *ctx) {
            ...
            for (auto &sys : _systems)
            {
                ...
                glm::vec3 motionDelta = sys.params.motion_delta_local;
                if (!std::isfinite(motionDelta.x) ||
                    !std::isfinite(motionDelta.y) ||
                    !std::isfinite(motionDelta.z))
                {
                    motionDelta = glm::vec3(0.0f);
                }

                pc.sim = glm::vec4(_dt_sec, _time_sec, drag, gravity);
                pc.origin_delta = glm::vec4(_origin_delta_local, motionDelta.z);
                ...
                pc.size_range = glm::vec4(minSize,
                                          maxSize,
                                          motionDelta.x,
                                          motionDelta.y);
                ...
                sys.reset = false;
                // Consume the per-frame delta so a stale value is never
                // re-applied when the game stops updating (e.g. paused).
                sys.params.motion_delta_local = glm::vec3(0.0f);
            }
        });
