...
void BackgroundPass::init_background_pipelines()
{
    ...
    createInfo.shaderPath = _context->getAssets()->shaderPath("background/gradient_color.comp.spv");
    ...
    createInfo.shaderPath = _context->getAssets()->shaderPath("background/sky.comp.spv");
    ...
    gp.vertexShaderPath = _context->getAssets()->shaderPath("common/fullscreen.vert.spv");
    gp.fragmentShaderPath = _context->getAssets()->shaderPath("background/background_env.frag.spv");
...
void BackgroundPass::register_graph(RenderGraph *graph, RGImageHandle drawHandle, RGImageHandle depthHandle)
{
    ...
    if (!_context->plainBackground && std::string_view(effect.name) == std::string_view("env"))
    {
        graph->add_pass(
            "BackgroundEnv",
            RGPassType::Graphics,
    ...
    else
    {
        graph->add_pass(
            ...
            [this, drawHandle](VkCommandBuffer cmd, const RGPassResources &res, EngineContext *ctx) {
                ...
                ComputePushConstants data = eff.data;
                if (ctx->plainBackground)
                {
                    data.data1 = glm::vec4(ctx->backgroundColor, 1.0f);
                ...
                dispatchInfo.pushConstants = &data;
                dispatchInfo.pushConstantSize = sizeof(ComputePushConstants);

                const char *instanceName = (ctx->plainBackground || std::string_view(eff.name) == std::string_view("gradient"))
                                           ? "background.gradient"
                                           : "background.sky";
