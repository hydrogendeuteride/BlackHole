                                        ...
                                        RGImageHandle cloudDepth)
{
    ...
    VkImageView cloud_depth_view = resources.image_view(cloudDepth);
    if (pos_view == VK_NULL_HANDLE || lut_view == VK_NULL_HANDLE ||
        cloud_lighting_view == VK_NULL_HANDLE || cloud_segment_view == VK_NULL_HANDLE || cloud_depth_view == VK_NULL_HANDLE)
    {
        return;
    }
    ...
    writer.write_image(3, noise_view, ctx->getSamplers()->linearRepeatClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(4, noise_view_3d, ctx->getSamplers()->defaultLinear(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
...
void AtmospherePass::draw_cloud_temporal(VkCommandBuffer cmd,
                                         ...
                                         RGImageHandle cloudDepthCurrent,
                                         ...
                                         const glm::vec4 &planetCenterRadius,
                                         bool historyValid)
{
    ...
    VkImageView cloud_depth_current_view = resources.image_view(cloudDepthCurrent);
    if (cloud_lighting_current_view == VK_NULL_HANDLE || cloud_segment_current_view == VK_NULL_HANDLE ||
        cloud_depth_current_view == VK_NULL_HANDLE)
    {
        return;
    }
    ...
    DescriptorWriter writer;
    writer.write_image(0, cloud_lighting_current_view, ctx->getSamplers()->nearestClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(1, cloud_segment_current_view, ctx->getSamplers()->nearestClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(2, cloud_lighting_history_prev_view, ctx->getSamplers()->linearClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(3, cloud_segment_history_prev_view, ctx->getSamplers()->nearestClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(4, cloud_depth_current_view, ctx->getSamplers()->nearestClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.update_set(ctx->getDevice()->device(), input_set);
    ...
    const float history_blend = ctx->planetClouds.resolutionDiv <= 1 ? 0.65f : 0.82f;
    push.origin_delta_blend = glm::vec4(originDeltaLocal, history_blend);
    ...
    push.planet_center_radius = planetCenterRadius;
    push.cloud_motion = glm::vec4(ctx->planetClouds.windSpeed, ctx->planetClouds.windAngleRad, 0.0f, 0.0f);
...
void AtmospherePass::draw_composite(VkCommandBuffer cmd,
                                    ...
                                    RGImageHandle gbufPos,
                                    RGImageHandle transmittanceLut,
                                    RGImageHandle skyViewLut,
                                    RGImageHandle skyViewOpticalDepthLut,
                                    RGImageHandle aerialLut,
                                    RGImageHandle aerialOpticalDepthLut,
                                    RGImageHandle cloudLightingResolved,
                                    RGImageHandle cloudSegmentResolved)
{
    EngineContext *ctx = context ? context : _context;
    ...
    VkImageView pos_view = resources.image_view(gbufPos);
    VkImageView lut_view = resources.image_view(transmittanceLut);
    VkImageView sky_view = resources.image_view(skyViewLut);
    VkImageView sky_od_view = resources.image_view(skyViewOpticalDepthLut);
    VkImageView aerial_view = resources.image_view(aerialLut);
    VkImageView aerial_od_view = resources.image_view(aerialOpticalDepthLut);
    ...
    if (sky_view == VK_NULL_HANDLE)
    {
        sky_view = assets->fallbackBlackView();
    }
    if (sky_od_view == VK_NULL_HANDLE)
    {
        sky_od_view = assets->fallbackBlackView();
    }
    if (aerial_view == VK_NULL_HANDLE)
    {
        aerial_view = _blackFallback3D.imageView;
    }
    if (aerial_od_view == VK_NULL_HANDLE)
    {
        aerial_od_view = _blackFallback3D.imageView;
    }
    if (overlay_view == VK_NULL_HANDLE || noise_view == VK_NULL_HANDLE || noise_view_3d == VK_NULL_HANDLE ||
        jitter_view == VK_NULL_HANDLE || sky_view == VK_NULL_HANDLE || sky_od_view == VK_NULL_HANDLE ||
        aerial_view == VK_NULL_HANDLE || aerial_od_view == VK_NULL_HANDLE ||
    ...
    writer.write_image(6, cloud_segment_resolved_view, ctx->getSamplers()->defaultNearest(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(7, noise_view_3d, ctx->getSamplers()->defaultLinear(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    ...
    writer.write_image(15, sky_view, ctx->getSamplers()->linearClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(16, sky_od_view, ctx->getSamplers()->linearClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(17, aerial_view, ctx->getSamplers()->linearClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(18, aerial_od_view, ctx->getSamplers()->linearClampEdge(),
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.update_set(ctx->getDevice()->device(), input_set);
