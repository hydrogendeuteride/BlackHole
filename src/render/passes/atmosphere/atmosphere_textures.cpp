...
void AtmospherePass::ensure_cloud_textures(EngineContext *context)
{
    ...
    bool uploaded = false;
    const auto retire = [&](AllocatedImage &image) {
        if (ctx->currentFrame)
            ctx->currentFrame->_deletionQueue.push_function([resources, old = image]() { resources->destroy_image(old); });
        else
    ...
    // Publish new sampled views only after copies and layout transitions complete.
    // Reload is infrequent; normal frames never wait here.
    if (uploaded) resources->process_queued_uploads_immediate();
}
