...
RGImageHandle AtmospherePass::register_graph(RenderGraph *graph, RGImageHandle hdrInput, RGImageHandle gbufPos)
{
    ...
    RGImageHandle sky_view_lut{};
    RGImageHandle sky_view_optical_depth_lut{};
    const bool run_sky_view_lut = _context->enableAtmosphere &&
        body_selection.radius_m > 0.0f &&
        atmosphere_radius > body_selection.radius_m &&
        settings.intensity > 0.0f;

    if (run_sky_view_lut)
    {
        RGImageDesc sky_view_desc{};
        sky_view_desc.name = "atmosphere.lut.sky_view";
        sky_view_desc.format = VK_FORMAT_R16G16B16A16_SFLOAT;
        sky_view_desc.extent = VkExtent2D{k_sky_view_lut_width, k_sky_view_lut_height};
        sky_view_desc.usage = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
        sky_view_lut = graph->create_image(sky_view_desc);

        RGImageDesc sky_od_desc{};
        sky_od_desc.name = "atmosphere.lut.sky_view_optical_depth";
        sky_od_desc.format = VK_FORMAT_R16G16B16A16_SFLOAT;
        sky_od_desc.extent = VkExtent2D{k_sky_view_lut_width, k_sky_view_lut_height};
        sky_od_desc.usage = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
        sky_view_optical_depth_lut = graph->create_image(sky_od_desc);

        const AtmosphereSkyViewPush sky_push = build_sky_view_push(*_context, body_selection);
        graph->add_pass(
            "AtmosphereLUT.SkyView",
            RGPassType::Compute,
            [transmittance_lut, sky_view_lut, sky_view_optical_depth_lut](RGPassBuilder &builder, EngineContext *)
            {
                builder.read(transmittance_lut, RGImageUsage::SampledCompute);
                builder.write(sky_view_lut, RGImageUsage::ComputeWrite);
                builder.write(sky_view_optical_depth_lut, RGImageUsage::ComputeWrite);
            },
            [this, transmittance_lut, sky_view_lut, sky_view_optical_depth_lut, sky_push](
                VkCommandBuffer cmd,
                const RGPassResources &resources,
                EngineContext *ctx)
            {
                EngineContext *ctx_local = ctx ? ctx : _context;
                if (!ctx_local || !ctx_local->pipelines || !ctx_local->getSamplers())
                {
                    return;
                }

                VkImageView sky_view = resources.image_view(sky_view_lut);
                VkImageView sky_od_view = resources.image_view(sky_view_optical_depth_lut);
                VkImageView transmittance_view = resources.image_view(transmittance_lut);
                if (sky_view == VK_NULL_HANDLE || sky_od_view == VK_NULL_HANDLE ||
                    transmittance_view == VK_NULL_HANDLE)
                {
                    return;
                }

                ctx_local->pipelines->setComputeInstanceStorageImage("atmosphere.sky_view_lut", 0, sky_view);
                ctx_local->pipelines->setComputeInstanceStorageImage("atmosphere.sky_view_lut", 1, sky_od_view);
                ctx_local->pipelines->setComputeInstanceSampledImage(
                    "atmosphere.sky_view_lut",
                    2,
                    transmittance_view,
                    ctx_local->getSamplers()->linearClampEdge());

                ComputeDispatchInfo dispatch = ComputeManager::createDispatch2D(
                    k_sky_view_lut_width,
                    k_sky_view_lut_height);
                dispatch.pushConstants = &sky_push;
                dispatch.pushConstantSize = sizeof(sky_push);

                ctx_local->pipelines->dispatchComputeInstance(cmd, "atmosphere.sky_view_lut", dispatch);
            });
    }

    RGImageHandle aerial_lut{};
    RGImageHandle aerial_optical_depth_lut{};
    if (run_sky_view_lut)
    {
        const VkExtent2D aerial_extent{k_aerial_lut_res, k_aerial_lut_res};

        RGImageDesc aerial_desc{};
        aerial_desc.name = "atmosphere.lut.aerial";
        aerial_desc.format = VK_FORMAT_R16G16B16A16_SFLOAT;
        aerial_desc.extent = aerial_extent;
        aerial_desc.depth = k_aerial_lut_slices;
        aerial_desc.usage = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
        aerial_lut = graph->create_image(aerial_desc);

        RGImageDesc aerial_od_desc{};
        aerial_od_desc.name = "atmosphere.lut.aerial_optical_depth";
        aerial_od_desc.format = VK_FORMAT_R16G16B16A16_SFLOAT;
        aerial_od_desc.extent = aerial_extent;
        aerial_od_desc.depth = k_aerial_lut_slices;
        aerial_od_desc.usage = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
        aerial_optical_depth_lut = graph->create_image(aerial_od_desc);

        const AtmosphereAerialPush aerial_push = build_aerial_push(*_context, body_selection);
        graph->add_pass(
            "AtmosphereLUT.Aerial",
            RGPassType::Compute,
            [transmittance_lut, aerial_lut, aerial_optical_depth_lut](RGPassBuilder &builder, EngineContext *)
            {
                builder.read(transmittance_lut, RGImageUsage::SampledCompute);
                builder.write(aerial_lut, RGImageUsage::ComputeWrite);
                builder.write(aerial_optical_depth_lut, RGImageUsage::ComputeWrite);
            },
            [this, transmittance_lut, aerial_lut, aerial_optical_depth_lut, aerial_push](
                VkCommandBuffer cmd,
                const RGPassResources &resources,
                EngineContext *ctx)
            {
                ...
                VkImageView aerial_view = resources.image_view(aerial_lut);
                VkImageView aerial_od_view = resources.image_view(aerial_optical_depth_lut);
                VkImageView transmittance_view = resources.image_view(transmittance_lut);
                if (aerial_view == VK_NULL_HANDLE || aerial_od_view == VK_NULL_HANDLE ||
                    transmittance_view == VK_NULL_HANDLE)
                {
                    return;
                }

                ctx_local->pipelines->setComputeInstanceStorageImage("atmosphere.aerial_lut", 0, aerial_view);
                ctx_local->pipelines->setComputeInstanceStorageImage("atmosphere.aerial_lut", 1, aerial_od_view);
                ctx_local->pipelines->setComputeInstanceSampledImage(
                    "atmosphere.aerial_lut",
                    2,
                    transmittance_view,
                    ctx_local->getSamplers()->linearClampEdge());
                ...
                ComputeDispatchInfo dispatch = ComputeManager::createDispatch2D(
                    k_aerial_lut_res,
                    k_aerial_lut_res);
                dispatch.pushConstants = &aerial_push;
                dispatch.pushConstantSize = sizeof(aerial_push);

                ctx_local->pipelines->dispatchComputeInstance(cmd, "atmosphere.aerial_lut", dispatch);
            });
    }
    ...
    const VkExtent2D low_extent = scaled_extent(draw_extent, cloud_resolution_div(_context->planetClouds.resolutionDiv));
    ...
    next_snapshot.cloudExtinction = _context->planetClouds.extinction;
    next_snapshot.cloudTint = _context->planetClouds.color;
    next_snapshot.cloudCoverage = _context->planetClouds.coverage;
    next_snapshot.cloudTypeBias = _context->planetClouds.typeBias;
    ...
    next_snapshot.cloudWeatherEvolutionPeriodS = _context->planetClouds.weatherEvolutionPeriodS;
    next_snapshot.cloudDetailEvolutionPeriodS = _context->planetClouds.detailEvolutionPeriodS;
    ...
    next_snapshot.cloudSteps = _context->planetClouds.cloudSteps;
    ...
    if (!history_reset)
    {
        history_reset = _historySnapshot.drawExtent.width != next_snapshot.drawExtent.width ||
                        ...
                        !nearly_equal(_historySnapshot.cloudExtinction, next_snapshot.cloudExtinction, 1.0e-6f) ||
                        !nearly_equal_vec3(_historySnapshot.cloudTint, next_snapshot.cloudTint, 2.0e-3f) ||
                        !nearly_equal(_historySnapshot.cloudCoverage, next_snapshot.cloudCoverage) ||
                        !nearly_equal(_historySnapshot.cloudTypeBias, next_snapshot.cloudTypeBias) ||
                        ...
                        !nearly_equal(_historySnapshot.cloudWeatherEvolutionPeriodS,
                                      next_snapshot.cloudWeatherEvolutionPeriodS) ||
                        !nearly_equal(_historySnapshot.cloudDetailEvolutionPeriodS,
                                      next_snapshot.cloudDetailEvolutionPeriodS) ||
                        ...
                        _historySnapshot.cloudSteps != next_snapshot.cloudSteps ||
    ...
    const glm::vec4 cloud_planet_center_radius = next_snapshot.planetCenterRadius;
    ...
    if (run_cloud_pipeline)
    {
        ...
        RGImageDesc low_depth_desc = low_segment_desc;
        low_depth_desc.name = "atmosphere.cloud.lowres_depth";
        low_depth_desc.format = VK_FORMAT_R32_SFLOAT;
        RGImageHandle cloud_depth_current = graph->create_image(low_depth_desc);
        ...
        graph->add_pass(
            ...
            RGPassType::Graphics,
            [gbufPos, transmittance_lut, cloud_lighting_current, cloud_segment_current, cloud_depth_current](RGPassBuilder &builder, EngineContext *)
            {
                ...
                builder.write_color(cloud_depth_current, true);
            },
            [this, low_extent, gbufPos, transmittance_lut, cloud_lighting_current, cloud_segment_current, cloud_depth_current](
                VkCommandBuffer cmd,
                const RGPassResources &resources,
                EngineContext *ctx)
            {
        ...
        graph->add_pass(
             ...
             cloud_segment_history_next](RGPassBuilder &builder, EngineContext *)
            {
                ...
                builder.read(cloud_depth_current, RGImageUsage::SampledFragment);
    ...
    graph->add_pass(
        ...
        [hdrInput,
         ...
         transmittance_lut,
         sky_view_lut,
         sky_view_optical_depth_lut,
         aerial_lut,
         aerial_optical_depth_lut,
         cloud_lighting_resolved,
         cloud_segment_resolved,
         hdr_output](
            ...
            EngineContext *)
        {
            ...
            builder.read(gbufPos, RGImageUsage::SampledFragment);
            builder.read(transmittance_lut, RGImageUsage::SampledFragment);
            if (sky_view_lut.valid()) builder.read(sky_view_lut, RGImageUsage::SampledFragment);
            if (sky_view_optical_depth_lut.valid()) builder.read(sky_view_optical_depth_lut, RGImageUsage::SampledFragment);
            if (aerial_lut.valid()) builder.read(aerial_lut, RGImageUsage::SampledFragment);
            if (aerial_optical_depth_lut.valid()) builder.read(aerial_optical_depth_lut, RGImageUsage::SampledFragment);
