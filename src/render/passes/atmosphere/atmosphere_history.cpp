...
void AtmospherePass::release_history_images(bool deferred)
{
    if (!_context || !_context->getResources())
    {
        return;
    ...
    for (AllocatedImage &image : _cloudLightingHistory)
    {
        ...
        {
            if (deferred && _context->currentFrame)
                _context->currentFrame->_deletionQueue.push_function([resources, old = image]() { resources->destroy_image(old); });
            else
