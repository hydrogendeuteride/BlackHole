...
namespace
{
    ...
    bool picks_share_outline_target(const PickingSystem::PickInfo &a, const PickingSystem::PickInfo &b)
    {
        if (!a.valid || !b.valid || a.kind != b.kind || a.kind != PickingSystem::PickInfo::Kind::SceneObject)
        {
            return false;
        }

        if (a.ownerType != b.ownerType || a.ownerName.empty() || b.ownerName.empty() ||
            a.ownerName != b.ownerName)
        {
            return false;
        }

        if (!a.objectName.empty() && !b.objectName.empty())
        {
...
void HoverOutlinePass::init(EngineContext *context)
{
    ...
    mask_info.vertexShaderPath = _context->getAssets()->shaderPath("materials/mesh.vert.spv");
    mask_info.fragmentShaderPath = _context->getAssets()->shaderPath("effects/hover_outline_mask.frag.spv");
    ...
    blur_info.vertexShaderPath = _context->getAssets()->shaderPath("common/fullscreen.vert.spv");
    blur_info.fragmentShaderPath = _context->getAssets()->shaderPath("effects/hover_outline_blur.frag.spv");
    ...
    composite_info.vertexShaderPath = _context->getAssets()->shaderPath("common/fullscreen.vert.spv");
    composite_info.fragmentShaderPath = _context->getAssets()->shaderPath("effects/hover_outline_composite.frag.spv");
...
RGImageHandle HoverOutlinePass::register_graph(RenderGraph *graph, RGImageHandle ldrInput, RGImageHandle depthHandle)
{
    ...
    const bool projected_selection = picking.selection_projection_active();
    const std::vector<PickingSystem::PickInfo> &selection_projection = picking.selection_projection();
    const PickingSystem::PickInfo &selection_pick = picking.last_pick();

    bool hover_suppressed = false;
    if (_settings.suppress_hover_when_selected)
    {
        if (projected_selection)
        {
            hover_suppressed = std::any_of(selection_projection.begin(),
                                           selection_projection.end(),
                                           [&](const PickingSystem::PickInfo &pick) {
                                               return picks_share_outline_target(hover_pick, pick);
                                           });
        }
        else
        {
            hover_suppressed = picks_share_outline_target(hover_pick, selection_pick);
        }
    }
    ...
    std::vector<const RenderObject *> selection_draws{};
    if (_settings.selection.enabled)
    {
        if (projected_selection)
        {
            for (const PickingSystem::PickInfo &pick : selection_projection)
            {
                const std::vector<const RenderObject *> draws =
                    collect_outline_draws(_context, draw_context, picking, pick, _settings.selection);
                selection_draws.insert(selection_draws.end(), draws.begin(), draws.end());
            }
        }
        else
        {
            selection_draws = collect_outline_draws(_context,
                                                     draw_context,
                                                     picking,
                                                     selection_pick,
                                                     _settings.selection);
            for (const PickingSystem::PickInfo &drag_pick : picking.drag_selection())
            {
                const std::vector<const RenderObject *> drag_draws =
                    collect_outline_draws(_context, draw_context, picking, drag_pick, _settings.selection);
                selection_draws.insert(selection_draws.end(), drag_draws.begin(), drag_draws.end());
            }
        }
    }
