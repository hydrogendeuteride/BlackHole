...
constexpr float kCsmDepthBiasConstant = 0.35f;
constexpr float kCsmDepthBiasSlope = 0.75f;
} // namespace
...
void ShadowPass::init(EngineContext *context)
{
    ...
    info.vertexShaderPath = _context->getAssets()->shaderPath("lighting/shadow.vert.spv");
    info.fragmentShaderPath = _context->getAssets()->shaderPath("lighting/shadow.frag.spv");
    ...
    info.configure = [this](PipelineBuilder &b) {
        ...
        // Shadow matrices use Vulkan's standard 0..1 depth range.
        b.enable_depthtest(true, VK_COMPARE_OP_LESS_OR_EQUAL);
        b.set_depth_format(VK_FORMAT_D32_SFLOAT);

        // Positive bias pushes casters deeper in standard-Z. Keep CSM lighter than
        // punctual lights to avoid detached contact shadows across broad cascades.
        b._rasterizer.depthBiasEnable = VK_TRUE;
        b._rasterizer.depthBiasConstantFactor = kCsmDepthBiasConstant;
        b._rasterizer.depthBiasSlopeFactor = kCsmDepthBiasSlope;
