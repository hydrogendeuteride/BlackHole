...
- Environment root: Honors `VKG_ASSET_ROOT` (expected to contain `assets/` and optionally generated `bin/shaders/`).
- Shader root: Honors `VKG_SHADER_ROOT` when generated shaders live outside the default runtime output tree.
- Upward search: If unset, searches upward from the current directory for `assets` and generated shaders.
- Fallbacks: Tries `./assets`, `../assets`, executable-adjacent `shaders/`, and detected shader roots.
...
auto spv = assets->shaderPath("materials/mesh.vert.spv");
