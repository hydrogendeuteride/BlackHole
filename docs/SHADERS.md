...
- Locations
  - Sources live under `shaders/`.
  - Generated SPIR-V files live under `bin/shaders/` by default, preserving the source-relative path.

- Build integration
  - CMake invokes `glslangValidator -V` for `*.vert`, `*.frag`, `*.comp` targeting Vulkan 1.2.
  - CMake writes generated files to `VULKAN_ENGINE_SHADER_OUTPUT_DIR`, defaulting to the runtime output directory's `shaders/` folder.
  - Helper `compile_shaders.py` uses `glslc` targeting Vulkan 1.3 and supports additional stages:
    ...
    - Use `--output-dir <dir>` to choose a generated shader directory or `--in-place` for the old source-adjacent layout.
...
- Conventions
  ...
  - Runtime shader lookup checks `VKG_SHADER_ROOT`, then `shaders/` beside the executable, then detected asset roots.
...
|------|-------|-------------|
| `common/fullscreen.vert` | Vertex | Fullscreen triangle for post-process passes |
| `materials/mesh.vert` | Vertex | Standard mesh vertex transform |
| `materials/mesh.frag` | Fragment | Forward shading (deprecated, use gbuffer) |
| `materials/gbuffer.frag` | Fragment | G-Buffer output (position, normal, albedo, AO+emissive) |
| `lighting/deferred_lighting.frag` | Fragment | Deferred lighting with RT shadows + IBL |
| `lighting/deferred_lighting_nort.frag` | Fragment | Deferred lighting without RT |
| `lighting/shadow.vert/.frag` | Vertex/Fragment | Shadow map generation |
| `post/ssr.frag` | Fragment | Screen-space reflections (ray march) |
| `post/ssr_rt.frag` | Fragment | SSR + RT fallback (ray query) |
| `post/tonemap.frag` | Fragment | HDR to LDR tonemapping + bloom |
| `post/fxaa.frag` | Fragment | Fast approximate anti-aliasing |
| `background/sky.comp` | Compute | Procedural sky background |
| `background/gradient_color.comp` | Compute | Gradient background |
| `background/background_env.frag` | Fragment | Environment map background |
| `materials/mesh_vfx.frag` | Fragment | Mesh VFX forward pass - animated noise, UV gradient, fresnel |
