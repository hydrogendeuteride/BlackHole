...
- `TextInputState`
  - UTF-8 text committed during the current frame.
  - Persistent IME composition text and selection range from Windows or Linux.
...
const TextInputState& text_input() const;     // UTF-8 commit/composition state
```
...
void set_text_input_area(int32_t x, int32_t y, int32_t width, int32_t height);
void clear_text_composition();
```

`committed_utf8` is cleared at the start of each frame. `composing_utf8` and
its selection range persist until the IME changes or commits the composition,
the window loses focus, or composition is explicitly cleared. The input area
uses logical window coordinates and positions native candidate UI on supported
Windows and Linux input methods.
...
void Game::update(InputSystem& input)
{
    ...
    // Camera look (visible drag capture)
...
**UTF-8 Text and IME Composition:**

```cpp
void TextBox::process_text(InputSystem& input)
{
    const TextInputState& text_input = input.text_input();
    text += text_input.committed_utf8;

    draw_composition(text_input.composing_utf8,
                     text_input.selection_start,
                     text_input.selection_length);
}
```

Do not construct text from key-down events: keyboard layouts and IMEs produce
text through SDL text events. Respect ImGui keyboard capture before consuming
the shared text state in gameplay UI.
...
- Use `set_mouse_capture(true)` for visible drag controls that should continue receiving mouse motion outside the window.
