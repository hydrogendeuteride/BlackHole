...
`third_party/imgui` is a Git submodule of [hydrogendeuteride/imgui](https://github.com/hydrogendeuteride/imgui), using the `vulkan-engine` branch. The engine pins a specific commit; normal submodule updates do not follow the branch automatically.

The migration preserves **1.90.6 WIP** (`IMGUI_VERSION_NUM = 19052`), based on upstream commit `a60387a3c25c18d223494873c7429d965691ea7f` (2024-04-29). All previously vendored files match that snapshot except the Vulkan clipping fix, kept as a separate fork commit (`62da360e30fbd5f0e74fcb71f92ce771d0d1f17f`). The backend now computes scissor sizes from rounded endpoints, avoiding a missing bottom/right pixel at fractional framebuffer scales.

After cloning or pulling the engine, run `git submodule update --init --recursive`. To upgrade ImGui, integrate and test upstream changes in the fork, push the resulting commit, then update and commit the engine's submodule pointer. Recheck HUD edges at 135% framebuffer scale, including letterboxed and odd-sized windows. Keep the clipping patch until an upstream equivalent is verified.
...
2. **Font Scaling**: Base font size (16px) is baked using DPI and logical layout scale.
3. **Window Resizing**: `FontGlobalScale` reuses the existing atlas, avoiding Vulkan texture and descriptor replacement during an ordinary resize.
4. **DPI Updates**: Fonts are rebuilt once when framebuffer DPI changes (for example, after moving the window between monitors) so text stays crisp.
