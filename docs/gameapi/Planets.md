...
struct PlanetCloudSettings
{
    ...
    // BC5: R = weather modulation, G = cloud type. R8 maps use a procedural type fallback.
    ...
    // Macro weather frequency + internal volume breakup
    float noiseScale{1.0f};
    float detailScale{12.0f};
    float noiseBlend{0.35f};
    float detailErode{0.60f};

    // Animation
    // Period 0 disables shape evolution for that layer
