...
1. `SceneManager::update_scene()`:
   ...
   - Computes cascaded shadow map matrices from camera frustum slices.
...
`update_scene()` computes frustum-slice CSM matrices for the directional (sun) light:

- `kShadowCascadeCount` cascade levels (default 4) split camera view depth from near plane `0.1` out to `kShadowCSMFar`.
- The split distribution uses a practical logarithmic/linear blend tuned toward near-field quality. With current defaults, cascade 0 covers roughly the first 15 view-space units before blending into cascade 1.
- Each cascade fits the camera frustum slice with a stable sphere, expands it by `kShadowCascadeRadiusScale` and `kShadowCascadeRadiusMargin`, then builds a light-space orthographic projection.
- Centers are snapped to the light-space texel grid for stable shadows.
- Shadow resolution is queried from `EngineContext::getShadowMapResolution()`.
- Depth uses standard 0..1 mapping (`glm::orthoRH_ZO`) paired with a `LESS_OR_EQUAL` depth test.
- Shaders choose cascades by camera view-depth split and blend near split boundaries; light-space containment is still used as a fallback guard.
