...
def parse_args():
    p = argparse.ArgumentParser(description="Compile GLSL shaders under shaders/ to generated .spv files.")
    ...
    p.add_argument("--output-dir", default="bin/shaders", help="SPIR-V output directory (default: bin/shaders).")
    p.add_argument("--in-place", action="store_true", help="Write .spv files next to shader sources.")
...
def compile_one(glslc: str, src: Path, shader_dir: Path, output_dir: Path, args) -> int:
    rel = src.relative_to(shader_dir)
    out_path = output_dir / rel.parent / f"{rel.name}.spv"
...
def main() -> int:
    ...
    failed = 0
    for src in sources:
        rc = compile_one(glslc, src, shader_dir, output_dir, args)
        if rc != 0:
            failed += 1
