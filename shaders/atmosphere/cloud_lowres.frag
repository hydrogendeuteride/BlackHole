...
layout(location = 2) out float outCloudDepth;
...
void main()
{
    ...
    mp.cloudEvolution = resolve_cloud_evolution_offsets();
    ...
    {
        float cloudStepScale = clamp(pc.cloud_animation.z, 0.25, 1.0);
        if (cloudStepScale > 0.99)
        {
            float cameraAltitude = max(length(camLocal - center) - planetRadius, 0.0);
            float nearQuality = 1.0 - smoothstep(750000.0, 2500000.0, cameraAltitude);
            cloudStepScale *= mix(2.0 / 3.0, 1.0, nearQuality);
        }
        cloudStepsTotal = clamp(int(round(float(cloudStepBudget) * cloudStepScale)),
                                min(CLOUD_LOWRES_CLOUD_STEP_MIN, cloudStepBudget),
                                clamp(pc.misc.z, 4, 128));
    }
    ...
    float cloudTransmittance = exp(-mp.cloudExtinction * state.odC);
    ...
    // Dark clouds still occlude the background, so retain an opacity-weighted fallback.
    outCloudDepth = state.cloudLightWeight > 1e-6
        ? state.cloudLightDepth / state.cloudLightWeight
        : (state.cloudOpacityWeight > 1e-6 ? state.cloudOpacityDepth / state.cloudOpacityWeight : 0.0);
