...
void main()
{
    ...
    ivec2 baseCoord = clamp(ivec2(inUV * vec2(lowSize)), ivec2(0), lowSize - ivec2(1));

    // At full resolution this pass is a resolve, not an upscale. Re-filtering
    // the 3x3 neighborhood here erases the volume detail we just raymarched.
    if (all(equal(lowSize, textureSize(posTex, 0))))
    {
        vec4 directSegment = texelFetch(cloudSegmentLowResTex, baseCoord, 0);
        if (segment_count(directSegment) > 0)
        {
            outCloudLighting = texelFetch(cloudLightingLowResTex, baseCoord, 0);
