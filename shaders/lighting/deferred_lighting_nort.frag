...
// Split smoothing width as a fraction of each camera-depth cascade range.
const float SHADOW_SPLIT_BLEND_FRACTION = 0.08;
...
const float SHADOW_SLOPE_BIAS = 0.00035;
...
float fetchCascadeDepthTexel(uint ci, ivec2 p, ivec2 dim)
{
    if (p.x < 0 || p.y < 0 || p.x >= dim.x || p.y >= dim.y)
    {
        return 1.0;
    }
    return texelFetch(shadowTex[ci], p, 0).r;
}

float sampleCascadeCompare(uint ci, vec2 uv, float compareDepth)
{
    ...
    vec2 p = uv * vec2(dim) - vec2(0.5);
    ivec2 p0 = ivec2(floor(p));
    vec2 f = fract(p);

    float s00 = step(compareDepth, fetchCascadeDepthTexel(ci, p0, dim));
    float s10 = step(compareDepth, fetchCascadeDepthTexel(ci, p0 + ivec2(1, 0), dim));
    float s01 = step(compareDepth, fetchCascadeDepthTexel(ci, p0 + ivec2(0, 1), dim));
    float s11 = step(compareDepth, fetchCascadeDepthTexel(ci, p0 + ivec2(1, 1), dim));

    return mix(mix(s00, s10, f.x), mix(s01, s11, f.x), f.y);
}

vec3 shadowReceiverNormal(vec3 localPos, vec3 shadingN)
{
    ...
    ng *= inversesqrt(len2);
    return (dot(ng, shadingN) < 0.0) ? -ng : ng;
}

// Compute primary cascade and an optional neighbor for cross-fade near borders
struct CascadeMix { uint i0; uint i1; float w1; };

bool insideCascade(uint ci, vec3 localPos)
{
    vec4 lclip = sceneData.lightViewProjCascades[ci] * vec4(localPos, 1.0);
    vec3 ndc = lclip.xyz / max(lclip.w, 1e-6);
    return abs(ndc.x) <= 1.0 && abs(ndc.y) <= 1.0 && ndc.z >= 0.0 && ndc.z <= 1.0;
}

CascadeMix computeCascadeMix(vec3 localPos)
{
    float depth = max(-(sceneData.view * vec4(localPos, 1.0)).z, 0.0);
    uint primary = 0u;
    if (depth > sceneData.cascadeSplitsView.x)
    {
        primary = (depth <= sceneData.cascadeSplitsView.y) ? 1u :
        ((depth <= sceneData.cascadeSplitsView.z) ? 2u : 3u);
    }

    if (!insideCascade(primary, localPos))
    {
        for (uint i = primary + 1u; i < 4u; ++i)
        {
            if (insideCascade(i, localPos))
            {
                primary = i;
                break;
            }
    ...
    if (primary < 3u)
    {
        float split0 = (primary == 0u) ? 0.0 : sceneData.cascadeSplitsView[int(primary) - 1];
        float split1 = sceneData.cascadeSplitsView[int(primary)];
        float band = max(0.5, (split1 - split0) * SHADOW_SPLIT_BLEND_FRACTION);
        float t = clamp((depth - (split1 - band)) / band, 0.0, 1.0);
        float w = smoothstep(0.0, 1.0, t);

        if (w > 0.0)
        {
            uint neighbor = primary + 1u;
            if (insideCascade(neighbor, localPos))
            {
                cm.i1 = neighbor;
                cm.w1 = w;
            }
...
float sampleCascadeShadow(uint ci, vec3 localPos, vec3 N, vec3 L)
{
    ...
    float slopeBias = max(SHADOW_SLOPE_BIAS * (1.0 - NoL), SHADOW_MIN_BIAS);
    ...
    int tile = int(mod(gl_FragCoord.x, 4.0)) + int(mod(gl_FragCoord.y, 4.0)) * 4;
    float ang = float((tile * 5) % 16) * 0.3926990817;
    ...
    for (int i = 0; i < TAP_COUNT; ++i)
    {
        ...
        float vis  = sampleCascadeCompare(ci, suv + off, currentBias - rpdb);
...
float calcShadowVisibility(vec3 localPos, vec3 N, vec3 L)
{
...
void main(){
    ...
    vec3 Ns = shadowReceiverNormal(pos, N);
    ...
    float sunVis = calcShadowVisibility(pos, Ns, Lsun);
    vec3 sunBRDF = evaluate_brdf(N, V, Lsun, albedo, roughness, metallic);
    vec3 direct = sunBRDF * sceneData.sunlightColor.rgb * sceneData.sunlightColor.a * sunVis * terrainSunVis * cloud_shadow_visibility(pos);
